...
## 13. 新主线程可直接使用的 Goal Prompt

在以 `F:\官媒投稿-refactor` 为项目根目录的新 Codex 线程中输入 `/goal`，然后粘贴以下内容：

```text
目标：严格按照 F:\官媒投稿-refactor\ARTICLE-LIFECYCLE-GOAL-ORCHESTRATION.md，持续完成文章生命周期与投稿流程重构的自动编排，从波次 1 完整集成审计开始，依次推进至 ticket 25 最终验收。只有文档中的全部完成条件满足时才能结束目标。
