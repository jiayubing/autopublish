...
4. 关闭明文传输、原始诊断和异常退出临时文件造成的秘密泄漏面。
...
| P1 | 安全、远端事实、数据一致性、灾备或破坏性操作风险 | 13 |
| P2 | 明确正确性/容量/恢复收益，且不阻塞 P1 基础 | 8 |
| P3 | 低风险 UX、契约或长期维护改进 | 4 |
| 待决策 | 必须先取得外部/业务架构决定 | 2 |
...
OPT-008、OPT-029 需要人工决策；OPT-028 暂缓
```
...
- 状态：需要决策
- 关联发现：F-H11
- 目标：API key、正文、余额和订单响应在跨主机传输中有可信 TLS；开箱默认不再允许公网明文 HTTP。
- 非目标：不自签或运营证书，不假定 VPN 等同 TLS，不改变媒体业务协议。
- 当前问题：默认公网 HTTP 被豁免确认并携带敏感 multipart。
- 根本原因：历史连接地址被当作“批准默认”绕过安全 interface。
...
- 前置依赖：人工确认生产 HTTPS endpoint、证书/DNS/代理或明确受控专网方案；OPT-001。
- 与其他优化项的关系：应在任何媒体功能正式发布前完成；与 OPT-009/014 不冲突。
- 推荐设计：默认无 endpoint 或使用已验证 HTTPS；非 loopback HTTP 一律拒绝，不能用普通“确认”永久绕过；若短期专网例外，使用受控 feature flag、到期日和现场证据。
- 实施步骤：确认拓扑；部署/验证 TLS；修改默认和校验；提供旧 HTTP 配置迁移错误；连接测试验证证书；安全抓包；移除豁免测试。
- 兼容性考虑：旧 HTTP 用户升级后会被阻断，必须提前公告并提供 HTTPS endpoint；不可静默回退。
- 数据迁移要求：配置迁移只标记/拒绝旧 endpoint，不复制 API key 到明文。
- 安全影响：关闭凭据/稿件窃听篡改主路径。
- 性能影响：TLS 握手小幅开销，可连接复用；需现场延迟测量。
- 测试要求：配置单元、安全测试、证书错误、MITM/抓包手工验证、外部集成。
- 验收标准：默认配置不会产生 HTTP 请求；所有生产媒体请求为证书验证成功的 HTTPS；错误证书/HTTP 明确拒绝且不发送 body。
- 发布或启用方式：先服务端 TLS，后客户端 fail-closed；设置兼容截止日期。
- 回滚方案：回滚业务发布，不恢复公网 HTTP；只能切换到已批准备用 HTTPS endpoint。
- 潜在回归风险：外部服务尚无 HTTPS导致媒体能力短期不可用。
- 复杂度：中
- 是否可以并行：是
- 并行限制：代码可在 endpoint 决策后与其他媒体项并行；上线顺序必须服务端先行。
...
- 包含：OPT-007、OPT-010、OPT-011、OPT-012；OPT-008仅完成决策/外部TLS准备。
- 为什么：这些项可在不改变远端事实核心状态机前关闭制品不可用、灾备假阳性、秘密落盘和删除TOCTOU。
- 前置条件：批次0完成；敏感fixture/隔离备份路径获授权；OPT-008取得endpoint方案才可实施。
- 执行顺序：007/010/011可并行 → 012 → production/DR/security联合验收；008服务端先于客户端。
...
- 失败时停止条件：任何测试接触真实生产数据、清理范围无法严格证明或HTTPS会静默回退时停止。
- 回滚策略：保留备份/旧记录，关闭相关功能；不回退到公网HTTP、原始截图或宽路径清理。
...
- OPT-008/029在决策前只允许只读现场核对和fixture准备；不得猜测生产拓扑或账号模型。
...
4. 安全项回滚不得恢复公网明文HTTP、原始页面截图或无边界临时文件清理。
