...
| OPT-008 | P1 / 可实施 | F-H11 | [ ] | [ ] | [ ] | [ ] | [ ] | D-001已决策：无隐式endpoint；媒体HTTP要求显式确认并持续警告 |
...
- [ ] 按已决策的D-001实施OPT-008：媒体HTTP例外仅通过显式endpoint和`allowInsecure`确认启用。
...
- [ ] OPT-008覆盖HTTP未确认拒绝、已确认允许、HTTPS无需确认、3xx不跟随、持续风险状态和无静默降级。

### 完成条件

- [ ] OPT-007、008、010、011、012 的五类证据全部完成。
- [ ] 最终制品脚本可执行；restore-check 对缺失/损坏目标零副作用；敏感工件验收通过；旧token不能删除新内容。
- [ ] 若测试触及真实生产数据、清理边界不能证明、HTTP绕过确认或HTTPS会静默降级，已立即停止并保留备份/记录。
...
- [ ] OPT-008已按D-001完成实现与残余风险记录；OPT-029已有人工决策和实施结果或明确不进入本轮范围；OPT-028已有产品决定或继续经批准暂缓。
