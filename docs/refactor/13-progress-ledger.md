...
| 当前可执行阶段 | 阶段6（`IN_PROGRESS`；整改完成，等待最终独立只读审计）       |
| 普通功能开发   | Phase 03/04/06保持`IN_PROGRESS`；Phase 07 `NOT_STARTED`；正式release仍冻结 |
| 正式release    | 冻结                                                      |
...
| 3 PublicationWorkflow | IN_PROGRESS | `7d8f81452f98c8211308ada0ffba7873428a764b` | — | Phase 03 owner findings与legacy删除已复核；13k临时SQLite为query/SQL=1/1、parsed=3、paidSendCalls=0。整改完成，等待最终独立只读审计。 | Phase 04人工项继续阻止正式release | `docs/refactor/handoffs/phase-03.md` |
|   4 Platform/Adapters | IN_PROGRESS | `8cbce7f1761c4e67baf4467d89f0a8397e93d9db` | — | P1-01与legacy media preflight删除已复核；platform/media inventory及event生命周期完整。整改完成，等待最终独立只读审计。 | 四项受控人工验收继续阻止正式release | `docs/refactor/handoffs/phase-04.md` |
...
|        6 Renderer/IPC | IN_PROGRESS | `743571d9597ea2c68ab10a08da0914ccaed5352b` | — | P2-09纠正后：npm test1255/1255，inventory109/109，packaged source/import/ASAR 5/5，最新Renderer focus1/1。整改完成，等待最终独立只读审计。 | Phase 03/04保持`IN_PROGRESS`；Phase 07仍为`NOT_STARTED` | `docs/refactor/handoffs/phase-06.md` |
...
- 状态：IN_PROGRESS（审计整改检查点A完成；自动门禁与最小解包目录制品 smoke 的既有证据保留，四项人工验收继续阻止正式release。）
