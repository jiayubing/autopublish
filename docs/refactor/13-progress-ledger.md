...
|  8 Cleanup/Acceptance | IN_PROGRESS   | `aff1dfd089aff2492f9054747ce55f94304cffdd` | Ticket 01/02 固化 commit                   | Ticket 01/02 已完成；production `src → desktop`=0，且架构门禁按 importer 解析仓库相对路径后分类并由3/3回归保护；workspace/path26/26、runtime/packaging/Hepan/diagnostics26/26、packaging46/46、links181/181、diagnostics32/32、扩展architecture82/82；lint与三套typecheck通过；其余 Ticket 03–17 cleanup/acceptance尚未执行 | Phase 04人工项与正式release继续`PENDING_HUMAN`/`BLOCKED_RELEASE`；完整root suite、全链验收、容量/迁移/准入模拟尚未执行 | `docs/refactor/handoffs/phase-08.md`、`docs/refactor/phase-08-decision-map.md` |
