...
| 当前可执行阶段 | 阶段7（`NOT_STARTED`；本任务未启动）                         |
| 普通功能开发   | Phase 06 已完成；Phase 07 未启动；正式release仍冻结          |
| 正式release    | 冻结                                                      |
...
|        6 Renderer/IPC | COMPLETE | `743571d9597ea2c68ab10a08da0914ccaed5352b` | 本次Phase 06里程碑commit（hash以Git历史为准） | 129/129 typed IPC；production sandbox preload、workspace bootstrap双解码、AI生成/豆包安全DTO、Unicode业务identity、workspace级command门禁、passive session、relaunch环境污染、generation preview projector、management cancellation-plan/legacy publication/reference-bound read-model、parsed invalidation及按订单identity安全打开发布证据均已收口；单篇伪通用导出下拉及caller已删除；最新221文件1232/1232；source/packaged真实调用、packaged ASAR 3/3、Auth16/16、links180/180、packaging33/33；lint、三套typecheck、format、Renderer/preload build、pack smoke、Electron focus1/1、diff check均通过 | Phase 04人工项仍只阻止正式release                          | `docs/refactor/handoffs/phase-06.md` |
