...
列举网浏览器登录的恢复规则：打开登录与检查登录使用同一恢复入口。已打开的浏览器保留当前登录、退出或换号操作；冷启动先核验持久浏览器中的登录，已登录时不导入保存的会话，确认未登录时才尝试恢复。检测异常不授权覆盖浏览器会话，也不等于未登录。历史会话损坏或恢复失败时明确提示，并保留浏览器供用户手动登录；检查成功仍须完成会话保存，账号绑定继续独立核验保存会话中的远端身份，禁止静默改绑。
