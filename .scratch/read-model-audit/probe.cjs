...
(async () => {
  for (const count of [100, 1000]) {
    ...
    try {
      const content = createContentStore({ articleStore: createArticleStore(root, { fs: io }), listClientIds: () => ['a', 'b'] });
      ...
      const makeAttention = () => createArticleAttentionQuery({ getRevision: invalidation.getRevision, readers: {
        listArticles: content.listArticles, listTrashedArticles: content.listTrashedArticles,
      ...
      assert.equal(await measure('title-cold', () => content.getGenerationTaskArticleTitle('task-a-0')), 'Title 0');
      assert.equal(await measure('title-hit', () => content.getGenerationTaskArticleTitle('task-a-0')), 'Title 0');
      const attention = makeAttention();
      assert.equal((await measure('attention-empty-cold', () => attention.list({ clientId: 'a' }))).items.length, 0);
      const management = createArticleManagementSnapshot({ listArticles: content.listArticles, listTrash: content.listTrashedArticles,
        getRevision: invalidation.getRevision, operationalStore: facts, articleAttentionQuery: makeAttention() });
      ...
      assert.equal(snapshot.articles[0].content.length, 4096);
