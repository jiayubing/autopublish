...
| Maintenance M06             | `RUNNING`   | M06-0 authoritative inventory/classification 与 `A → B → C → D → E → F → G` scope freeze 已完成；M06-A、M06-B、M06-C、M06-D COMPLETE、M06-E READY；M06-G 完成前不得关闭维护 10.5 或启动 Ticket 25 |

**当前动作：M05=`COMPLETE`。M06-0 已完成 authoritative inventory/classification 与 `A → B → C → D → E → F → G` scope freeze；M06-A、M06-B、M06-C、M06-D 已分别完成实现、定向验证、Primary Audit、blocking remediation、bounded re-audit 与 inventory 对账，M06-D 证据见 `handoffs/M06-D-optional-probe-parse-diagnostics-ipc-renderer-cleanup.md`。当前状态为 M06-A=`COMPLETE`、M06-B=`COMPLETE`、M06-C=`COMPLETE`、M06-D=`COMPLETE`、下一 gate M06-E=`READY`。维护 10.5 仍为 `PARTIAL`；M06-G 完成最终 combined audit、inventory reconciliation 与 clean-HEAD full gate 前不得关闭 10.5，Ticket 25 仍 `PENDING`/blocked by M06。本次未 push、release 或执行真实外部操作。**
...
| 10.5 | M04-A → M04-B → M04-C → M05-0 → A → B → C → D → E1 → E2 → E3 → F → G → H → I → J → J3 → J4 → J5 → J6 → J7 → J8 → J9 → M06-0 → M06-A → M06-B → M06-C → M06-D → M06-E → M06-F → M06-G | Wave 10 COMPLETE；M05-0 已将 E 冻结为 `E1 → E2 → E3`；M06-0 已冻结 M06 A–G | `PARTIAL`；M04/M05/M06-0/M06-A/M06-B/M06-C/M06-D COMPLETE；M06-E READY；M06-G 前 10.5 不得 COMPLETE |
