...
**当前动作：Ticket 25=`READY`；`25-0 — Startup Readiness` 已完成并记录在 `handoffs/25-0-startup-readiness.md`，下一执行包为 `25-A — Acceptance Contract, State Matrix & Evidence Baseline`。当前固定采用 Manual Dispatch package-by-package，尚未授权 Continuous Goal；每次只可调度最左 gate 已满足包。若未来要求持续执行到 COMPLETE，必须先解决 `EXECUTION-PROTOCOL.md` 1.3 与 Ticket 25 单次 combined-audit closure 的冲突。启动准备记录的 base integration HEAD 为 `292a2f4`，其 docs-only preparation commit 为 `cb6711a`；真正调度 25-A 时仍必须以实际 Git HEAD/status 为准，不复用任一旧 HEAD generated evidence。Ticket 25 尚未进入实现/验收执行，未执行真实登录、发布、付费或平台操作。**
...
- **Wave 11**：当前固定 Manual Dispatch package-by-package，按 `25-A → B → C → D → E → F → G` 严格串行；一次用户指令只授权当前最左 gate 已满足包，不自动推进下一包。包内只做定向验证和 combined audit evidence，不各自开启 fresh full audit。Continuous Goal 未授权；若未来切换，必须先显式协调 `EXECUTION-PROTOCOL.md` 1.3 的一包一任务/包级 audit-commit evidence 与本 Ticket 单次 combined audit 模式，并同步权威合同。G 之后由用户另派一次 Ticket 25 / Wave 11 独立 combined audit，blocking remediation 后只做 bounded closure re-audit；授权 commit/merge 后在最终 clean integration HEAD 运行正式 smoke。真实平台发布、付费和订单刷新仍需逐次用户授权；缺少核心外部 evidence 时保持 `BLOCKED/USER_EXTERNAL_ACCEPTANCE_REQUIRED`。
