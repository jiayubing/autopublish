...
**当前动作：Ticket 25=`RUNNING`；`25-0 — Startup Readiness`、`25-A`、`25-B`、`25-C`、`25-D — Paid Media / Order Acceptance`、`25-E — Migration / Recovery Acceptance`、`25-F — Performance & Responsibility Gates` 均已完成 package closure；25-F 已由主任务从 `de72d734c47baca3129ddf43ee182eaa49a866f1` fast-forward 集成到当前真实 clean integration HEAD `72ba6e136977f089405e9a1993747e368e0f8615`，并在该 HEAD 重跑 F benchmark/contract、直接测试、architecture/dependency、capacity、discovery、lint、format 和 diff check 全部通过。下一合同包为 `25-G`，当前尚未调度；只有本次主任务完成 25-G package closure、Independent Audit Handoff、集成和状态更新后停止。不得进入 G 后 Independent Combined Audit、remediation、bounded re-audit、final clean smoke、真实外部验收或 Wave 11 `COMPLETE`；禁止 push。Ticket 25 的 combined audit 仍按自身合同延后至 `25-G` 后，本次 Goal 不执行它。A～F 的实现、定向 gate、handoff 和 merge evidence 已核验；不复用旧 HEAD generated evidence。该调度与停止边界见本轮 handoff `handoffs/25-f-g-goal-mode-dispatch-reconciliation.md`。Ticket 25 尚未执行真实登录、发布、付费或平台操作。**
...
| 11   | 25-0 → 25-A → 25-B → 25-C → 25-D → 25-E → 25-F → 25-G → Independent Combined Audit → Remediation → Bounded Closure Re-audit → authorized commit/merge → final clean smoke → authorized external acceptance | 24；10.5 COMPLETE（含 M06）；各包严格依赖左侧已验证 sourceState；真实外部操作逐次授权 | `RUNNING`；25-0、25-A、25-B、25-C、25-D、25-E、25-F COMPLETE，25-F 已集成到 `72ba6e1`，下一合同包为 25-G 且尚未调度；G package closure、Independent Audit Handoff、集成和状态更新后停止，不进入后续 closure |
...
- **Wave 11**：Ticket 25 的合同顺序仍是 `25-A → B → C → D → E → F → G`，包内只收集定向验证和 combined-audit 所需 evidence，不各自开启 fresh full audit；`25-G` 之后再由独立任务执行一次 Ticket 25 / Wave 11 combined audit。对本次用户 Goal，`EXECUTION-PROTOCOL.md` 1.3 覆盖 `25-F → 25-G`：25-F 已由主任务核验并集成到 `72ba6e1`，当前只可创建一个新的用户可见 `luna`/`max` 25-G 执行任务；主任务必须等待其实现、定向测试和 evidence 返回，核验后集成到新的 clean HEAD，再完成状态更新。不得预建或并行 25-G。Goal 在 25-G package closure、Independent Audit Handoff、集成和状态更新后停止，不能进入 combined audit、remediation、bounded closure re-audit、final clean smoke、真实外部验收或 Wave 11 final closure；因此不得把 25-G package closure 记为 Ticket 25/Wave 11 `COMPLETE`。真实平台发布、付费和订单刷新仍需逐次用户授权；缺少核心外部 evidence 时保持 `BLOCKED/USER_EXTERNAL_ACCEPTANCE_REQUIRED`。
