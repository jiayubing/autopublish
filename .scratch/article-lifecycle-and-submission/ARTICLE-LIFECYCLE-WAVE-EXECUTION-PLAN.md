...
| 12   | 18-0 → 18-A → 18-B → 18-C → 18-D → 18-E                | 08、09、10、17；Ticket 26 的 26-I 本地 closure；上述 Wave 12 调度例外 | `PARTIAL`；18-0、18-A 与 18-B Closure 已闭合；18-A evidence 见 `handoffs/18-A-queue-image-count-persistence.md`，18-B evidence 见 `handoffs/18-B-queue-image-config-surface.md`；18-C implementation/定向验证完成，等待 Primary Audit、closure 与授权 commit，见 `handoffs/18-C-client-image-plan-composition.md`；18-D 尚不可调度 |
