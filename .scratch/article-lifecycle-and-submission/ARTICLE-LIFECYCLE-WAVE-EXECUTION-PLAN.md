...
| 12   | 18-0 → 18-A → 18-B → 18-C → 18-D → 18-E                | 08、09、10、17；Ticket 26 的 26-I 本地 closure；上述 Wave 12 调度例外 | `PARTIAL`；18-0、18-A 与 18-B Closure 已闭合；18-A evidence 见 `handoffs/18-A-queue-image-count-persistence.md`，18-B evidence 见 `handoffs/18-B-queue-image-config-surface.md`；下一项仅 `18-C`                                                                 |
