...
**当前动作：Ticket 25=`RUNNING`；`25-0 — Startup Readiness`、`25-A`、`25-B`、`25-C`、`25-D — Paid Media / Order Acceptance`、`25-E — Migration / Recovery Acceptance`、`25-F — Performance & Responsibility Gates` 均已完成 package closure。25-G 执行包已在基线 `4c9dfe577f8e963006e7dd0a00859170e77fae23` 上完成 execution gates、必要的真实 owner 修复、专用 dirty smoke 和 Independent Audit Handoff 材料，implementation commit 为 `fa96f0868bb4ddff07ddd33fb632276a03044335`；当前待主任务核验并集成，不把该执行包写成 Ticket 25/Wave 11 `COMPLETE`。下一步只是在主任务集成后执行一次 Independent Combined Audit；本执行包不进入 remediation、bounded re-audit、final clean smoke、真实外部验收或 Wave 11 `COMPLETE`，禁止 push。25-G 的完整命令、失败修复、sourceState 和 evidence 见 `handoffs/25-G-execution-gates-and-independent-audit-handoff.md`。Ticket 25 尚未执行真实登录、发布、付费或平台操作。**
...
| 11   | 25-0 → 25-A → 25-B → 25-C → 25-D → 25-E → 25-F → 25-G → Independent Combined Audit → Remediation → Bounded Closure Re-audit → authorized commit/merge → final clean smoke → authorized external acceptance | 24；10.5 COMPLETE（含 M06）；各包严格依赖左侧已验证 sourceState；真实外部操作逐次授权 | `RUNNING`；25-0、25-A、25-B、25-C、25-D、25-E、25-F COMPLETE；25-G execution package 已完成并留下 `execution ready for independent audit` handoff，implementation 尚待主任务核验/集成；不进入后续 closure |
...
- **Wave 11**：Ticket 25 的合同顺序仍是 `25-A → B → C → D → E → F → G`，包内只收集定向验证和 combined-audit 所需 evidence，不各自开启 fresh full audit；`25-G` 之后再由独立任务执行一次 Ticket 25 / Wave 11 combined audit。25-G execution package 已完成并提交 handoff，主任务必须核验 implementation/docs/evidence provenance 后集成到新的 clean HEAD，再调度 Independent Combined Audit；不得把本 package closure 记为 Ticket 25/Wave 11 `COMPLETE`。本执行包不进入 remediation、bounded re-audit、final clean smoke 或真实外部验收；真实平台发布、付费和订单刷新仍需逐次用户授权，缺少核心外部 evidence 时保持 `BLOCKED/USER_EXTERNAL_ACCEPTANCE_REQUIRED`。
