...
**当前动作：Ticket 25=`PARTIAL`；`25-0 — Startup Readiness`、`25-A`、`25-B`、`25-C`、`25-D — Paid Media / Order Acceptance`、`25-E — Migration / Recovery Acceptance`、`25-F — Performance & Responsibility Gates` 与 `25-G` execution/package closure 均已完成。25-G implementation commit `fa96f0868bb4ddff07ddd33fb632276a03044335`、Independent Audit Handoff 材料和对应 docs 已由主任务核验 provenance，并集成到新的 clean integration HEAD `c9a4ed8d692da6fd4714e25e12e6086085640c7d`；本次 Goal 在状态同步后停止，不把该 package closure 写成 Ticket 25/Wave 11 `COMPLETE`。不得进入 Independent Combined Audit、remediation、bounded re-audit、final clean smoke、真实外部验收或 Wave 11 `COMPLETE`，禁止 push。25-G 的完整命令、失败修复、sourceState 和 evidence 见 `handoffs/25-G-execution-gates-and-independent-audit-handoff.md`。Ticket 25 尚未执行真实登录、发布、付费或平台操作。**
...
- **Wave 11**：Ticket 25 的合同顺序仍是 `25-A → B → C → D → E → F → G`，包内只收集定向验证和 combined-audit 所需 evidence，不各自开启 fresh full audit；`25-G` 之后再由独立任务执行一次 Ticket 25 / Wave 11 combined audit。25-G execution package、Independent Audit Handoff 与主任务侧 provenance 核验已完成，G implementation/docs 已集成到 clean integration HEAD `c9a4ed8`；本次 Goal 在 package closure、handoff、集成和状态更新后停止，不得把本 package closure 记为 Ticket 25/Wave 11 `COMPLETE`。本执行包不进入 Independent Combined Audit、remediation、bounded re-audit、final clean smoke 或真实外部验收；真实平台发布、付费和订单刷新仍需逐次用户授权，缺少核心外部 evidence 时保持 `BLOCKED/USER_EXTERNAL_ACCEPTANCE_REQUIRED`。
