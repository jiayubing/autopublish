...
| Maintenance M06             | `RUNNING`   | post-closure audit 在 `696f5cff` 暴露 1 个 P1、2 个 blocking P2；production failure-semantics remediation 与定向 bounded re-audit 已完成，仍等待获授权提交后保存 post-commit clean-HEAD full gate evidence；Ticket 25 保持 PENDING/blocked/not started |

**当前动作：M05=`COMPLETE`、M06/M06-G=`RUNNING`、Maintenance 10.5=`PARTIAL`。`696f5cff` 后续 audit 的两个 production failure-semantics finding 已在 dirty candidate 上完成最小根因修复，61/61 直接调用链回归、format 与 lint PASS；但 post-commit clean-HEAD full gate evidence 尚未形成，因此不得恢复 `COMPLETE`。当前 remediation evidence 见 `handoffs/M06-G-post-closure-blocking-remediation.md`。Ticket 25 保持 `PENDING`/blocked/not started；未 push、release 或执行真实外部操作。**
...
| 10.5 | M04-A → M04-B → M04-C → M05-0 → A → B → C → D → E1 → E2 → E3 → F → G → H → I → J → J3 → J4 → J5 → J6 → J7 → J8 → J9 → M06-0 → M06-A → M06-B → M06-C → M06-D → M06-E → M06-F → M06-G | Wave 10 COMPLETE；M06-G post-closure blocking remediation 正在收口；post-commit clean-HEAD evidence 未完成 | `PARTIAL`；M04/M05 与 M06-A～F 保持历史 COMPLETE，M06-G remediation 为 RUNNING |
