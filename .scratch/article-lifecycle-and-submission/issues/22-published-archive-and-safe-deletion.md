...
1. 定义发布档案只读模型：复用 `publicationEvidenceV1` 的文章身份、客户安全快照、实际投稿标题/正文快照或不可得标记、content fingerprint、图片安全 fingerprint/布局/降级摘要、目标、账号/媒体快照、`submittedAt`、`firstPublishedAt`、时间来源/不可得原因、结果、订单号和安全远端链接；该模型只读，不从当前文章、图片库或浏览器会话重新推导/补写历史投稿内容与时间。
...
- [ ] 带图、换图和纯文本降级档案展示冻结 manifest 的安全摘要；图片文件后续变化/删除不会改写投稿证据，界面不暴露路径或二进制。
