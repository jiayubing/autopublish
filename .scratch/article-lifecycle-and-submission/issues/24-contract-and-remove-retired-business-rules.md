...
**Scheduling gate:** 作为独立波次 10，等待波次 9 Ticket 23 完成并使波次 9 `COMPLETE` 后调度；不等待后置 Ticket 18–21。
...
9. 保留 08/09 的封闭 evidence validator、纯文本实现和平台 adapter 窄端口，但生产 UI 不暴露尚未实现的图片入口；不得以“清理旧规则”为由删除后置 Ticket 18–21 所需的稳定扩展接缝，也不得提前实现图片。
