...
3. 通过 OperationalStore 的单个 admission 事务端口，为每篇成功项同时声明唯一活动目标并创建待执行队列项；内部投稿快照可用于审计，但不得作为可独立编辑文章暴露。
4. 提供单篇和批量移除命令，只允许尚未领取/尚未远端开始的项；通过 transition-specific removal 事务同时写入明确终态、移除队列项并结束对应活动目标，不调用通用 release。
