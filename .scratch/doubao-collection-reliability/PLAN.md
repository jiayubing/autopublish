...
## 2026-09-11 保存竞态局部续修（完成）

- 基线：155308b4；用户要求继续修复，保留现有节流。
- 确认 collectOne 在浏览器 await 后直接保存，可能覆盖期间人工更新或复活已删除问题的研究结果。
- 在同一服务 owner 中保存开始时的研究快照；写入前同步复查问题文本、启用状态和研究内容。变化时以稳定错误码失败，保留用户当前内容，不自动重试。没有新增持久状态、锁、schema 或浏览器调用。
- 新增 5 个故障注入用例，覆盖等待期间编辑、禁用、删除问题、人工保存与删除研究结果。现有用例继续覆盖无变化时正常采集和强制覆盖。
- 有界复查：直接调用方通过同一服务操作，复查到 saveResearch 间没有 await；失败由原队列失败路径处理，无新增 writer。未发现阻塞项。
- 最终验证：豆包相关 node --test 144/144；npm run lint、npm run typecheck:main 通过；git diff --check 通过。未运行真实账号采集和全仓测试，本次为局部同步保存保护，已覆盖相关测试集合。
- Git：修改保留工作区，未 stage/commit/push。
