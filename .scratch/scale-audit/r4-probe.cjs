...
async function probe() {
  ...
  try {
    if (mode === "runtime") {
      await measure("regular-global-identities", () => ports.regularQueueGroupTransitions.startAllRegularQueueGroups({runtimeOnly:true}));
      await measure("paid-global-identities", () => ports.paidExecutionTransitions.startAllPaidSubmissionBatches());
      await measure("regular-startup", () => ports.regularQueueGroupTransitions.pauseRegularQueueGroupsOnStartup());
      await measure("paid-startup", () => ports.paidExecutionTransitions.pausePaidSubmissionBatchesOnStartup());
      ...
      const service = createSubmissionCenterSnapshot({ getRevision:invalidation.getRevision, getCacheRevision:invalidation.getArticleReadRevision,
        ...
        listPaidMediaBatches:input=>store.listPaidSubmissionBatchSnapshots({...input,workbenchOnly:true}), listAttention: mode === "center-full" ? attentionQuery.list : ()=>({items:[]}) });
      ...
      await measure("center-page5000",()=>service.get({page:5000,pageSize:10}));
      await measure("center-page5001",()=>service.get({page:5001,pageSize:10}));
