...
export function summarizePublicationRecords(records: PublicationHistoryRecord[]): PublicationHistorySummary {
  ...
  const statuses = records.map((record) => record.status as PublicationRecordStatus);
...
export function publicationRecordStatusLabel(status: string | null | undefined): string {
