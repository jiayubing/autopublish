...
export const PUBLICATION_STATUS_LABELS: Record<
  PublicationHistorySummary["status"],
  string
> = {
  not_submitted: "未投稿",
  queued: "已入队",
...
export const PUBLICATION_STATUS_FILTERS = [
  ...
  { value: "queued", label: PUBLICATION_STATUS_LABELS.queued },
  {
    ...
    label: PUBLICATION_STATUS_LABELS.paid_processing,
  },
...
export type PublicationHistoryFilter =
  "all" | PublicationHistorySummary["status"];

const BLOCKING_STATUSES = new Set<PublicationRecordStatus>([
  "failed",
  "cancelled",
]);
...
function lifecycleStatusOf(
  ...
  const known = new Set<PublicationRecordStatus>([
    "queued",
    "remote_started",
    "paid_processing",
    "published",
  ...
  return known.has(record.status) ? record.status : "manual_check";
}
...
export function summarizePublicationRecords(
  records: PublicationHistoryRecord[],
): PublicationHistorySummary {
  if (!records.length)
    return {
      status: "not_submitted",
      label: PUBLICATION_STATUS_LABELS.not_submitted,
      records: 0,
      published: 0,
      uncertain: false,
    };
  ...
  if (statuses.includes("uncertain"))
    return {
      status: "uncertain",
      label: PUBLICATION_STATUS_LABELS.uncertain,
  ...
  if (published > 0 && published < records.length)
    return {
      status: "partial",
      label: PUBLICATION_STATUS_LABELS.partial,
      records: records.length,
      published,
      uncertain: false,
    };
  if (published === records.length)
    return {
      status: "published",
      label: PUBLICATION_STATUS_LABELS.published,
      records: records.length,
      published,
      uncertain: false,
    };
  if (statuses.includes("paid_processing"))
    return {
  ...
  if (statuses.every((status) => BLOCKING_STATUSES.has(status)))
    return {
      status: "failed",
      label: PUBLICATION_STATUS_LABELS.failed,
      records: records.length,
      published,
      uncertain: false,
    };
  if (statuses.includes("manual_check"))
    return {
      status: "manual_check",
      label: PUBLICATION_STATUS_LABELS.manual_check,
      records: records.length,
      published,
      uncertain: true,
    };
  if (statuses.includes("remote_started"))
    return {
      status: "queued",
      label: PUBLICATION_STATUS_LABELS.queued,
      records: records.length,
      published,
      uncertain: false,
    };
...
export function publicationSummaryMatchesFilter(
  summary: PublicationHistorySummary,
  filter: PublicationHistoryFilter,
): boolean {
...
export function publicationStatusLabel(
  ...
  return (
    PUBLICATION_STATUS_LABELS[status as PublicationHistorySummary["status"]] ||
    (status ? PUBLICATION_STATUS_LABELS.manual_check : "未知")
  );
}

export function publicationRecordStatusLabel(
  status: string | null | undefined,
  record?: Pick<
    PublicationHistoryRecord,
    "mediaResourceId" | "targetKey" | "platformId"
  > | null,
): string {
  return publicationStatusLabel(status);
}

export function publicationTargetLabel(
  record: PublicationHistoryRecord,
): string {
  if (record.mediaResourceId)
    return `${record.displayName || "付费媒体资源"} · 资源 ${record.mediaResourceId}`;
