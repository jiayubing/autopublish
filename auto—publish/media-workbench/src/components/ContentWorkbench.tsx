...
export default function ContentWorkbench({ attentionIntent, onAttentionIntentConsumed }: ContentWorkbenchProps) {
  ...
  const requestHistoryLeave = useCallback((action: () => void) => {
    if (!historyEditingArticle || !historyDirtyRef.current) { action(); return; }
    confirmationActionRef.current = action;
    setPendingConfirmation({ id: ++confirmationIdRef.current, clientId, kind: 'discard-history', title: '放弃未保存修改？', message: '历史文章有未保存修改，确认离开并放弃这些修改吗？', confirmLabel: '放弃修改' });
  }, [clientId, historyEditingArticle]);
  ...
  async function copyHistoryVersion() {
    ...
    confirmationActionRef.current = async () => { try {
      const copied = await copyContentArticleVersion({ clientId, sourceArticleId: sourceArticle.id });
    ...
    setPendingConfirmation({ id: ++confirmationIdRef.current, clientId, kind: 'copy-version', title: '确认复制文章新版本', message: `确认复制“${sourceArticle.title}”为新版本？原文章和发布记录不会修改。新版本会生成新的 articleId，必须重新选择目标并投稿。`, confirmLabel: '确认复制' });
  }

  function handleClientChange(nextClientId: string) {
    if (nextClientId === clientId) return;
    requestHistoryLeave(() => { closeHistoryEditor(true); setClientId(nextClientId); setArticle(null); setError('');
