...
export default function ContentWorkbench() {
  ...
  return <div className="content-workbench flex h-full min-h-0 flex-col overflow-hidden">
    <div className="flex shrink-0 items-center gap-2 border-b border-slate-200 bg-white px-4 py-3">
      {(['questions', 'generate', 'history'] as const).map((id) => <button id={id} type="button" key={id} onClick={() => changeTab(id)} className={`rounded-md px-3 py-2 text-xs font-semibold ${tab === id ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>{id === 'questions' ? '问题与采集' : id === 'generate' ? '文章生成' : '历史文章'}</button>)}
    ...
    <div className="min-h-0 flex-1">
      ...
      {tab === 'history' && <div className="flex h-full min-h-0 min-w-0 gap-3 p-3"><div className="min-h-0 min-w-0 flex-1"><GeneratedArticlesView clientId={clientId} refreshToken={articleRefreshToken} onArticleSelect={openHistoryEditor} onRefreshArticles={refreshArticles} /></div>{historyEditingArticle && <GeneratedArticleEditorPanel article={historyEditingArticle} published={historyEditingPublished} onSaved={(saved) => { setHistoryEditingArticle(saved); refreshArticles(); }} onClose={() => closeHistoryEditor(true)} onCopyVersion={() => void copyHistoryVersion()} onDirtyChange={(dirty) => { historyDirtyRef.current = dirty; }} />}</div>}
    </div>
  </div>;
}
