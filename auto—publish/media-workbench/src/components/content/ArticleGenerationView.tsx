...
export default function ArticleGenerationView({ clientId, client, clients = [], refreshToken, selectedArticle, onArticleChange, onRefresh }: ArticleGenerationViewProps) {
  ...
  async function retryMaterialItem(materialId: string) {
    ...
    try {
      ...
      setMaterialItems((current) => current.map((item) => (item.id || item.name) === materialId ? toMaterials({ knowledgeFiles: [nextMaterial], id: clientId, name: clientId })[0] : item));
