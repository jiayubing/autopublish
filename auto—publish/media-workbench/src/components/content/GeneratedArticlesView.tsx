...
import type { ArticleAttentionItem, ArticleRemovalTransaction, ArticleTrashImpactItem, ArticleTrashPreview, ArticleTrashRecord, ContentSubmissionBatchRecord, ContentSubmissionCancellationPreview, ContentSubmissionPlatform, PublicationHistoryRecord } from '../../types/publication';
...
export default function GeneratedArticlesView({ clientId, management, query, commands, commandStates, removal, watchRemovalTransaction, stageFilter = 'all', selectedAttentionId, onArticleSelect, onStageFilterChange }: GeneratedArticlesViewProps) {
  ...
  const commandBusy = useCallback((...names: string[]) => names.some((name) => commandStates[name]?.busy === true), [commandStates]);
  ...
  async function queueSelected() {
    ...
    try {
      ...
      setError('');
      try {
        const result = await commands.createContentSubmissionBatch({ ...input, confirmed: true });
  ...
  async function handoffSelectedToMedia() {
    ...
    try {
      ...
      for (const input of inputs) await commands.previewExport(input);
      if (!isCurrentClient(requestedClientId)) return;
      if (!(await confirm({
        title: '确认加入付费媒体投稿',
      ...
      setError(''); setBatchFeedback(null);
      try {
        for (const input of inputs) {
          const result = await commands.exportToSubmissionQueue(input);
  ...
  async function copyPublishedVersion() {
    ...
    setError(''); try {
      ...
      if (isContentCommandStaleResult(nextArticle) || !isCurrentClient(requestedClientId)) return;
  ...
  async function reconcilePublication(record: PublicationHistoryRecord, status: 'published' | 'failed') {
    ...
    setError(''); try {
      const result = await commands.reconcilePublication({ publicationId: record.publicationId, status, reasonCode: status === 'published' ? 'CONFIRMED_PUBLISHED' : 'CONFIRMED_NOT_PUBLISHED' });
      if (isContentCommandStaleResult(result) || !isCurrentClient(requestedClientId)) return;
  ...
  async function cancelCancelableBatches() {
    ...
    try {
      ...
      try {
        const results = [];
        for (const preview of previews) if (preview.allowedCount) {
          const result = await commands.cancelContentSubmissionBatch({ batchId: preview.batchId, planId: preview.planId });
          if (isContentCommandStaleResult(result)) return;
          results.push(result);
        }
  ...
  async function cleanupFailedBatches() {
    ...
    if (commandBusy('previewCleanupFailedContentSubmissionItems', 'cleanupFailedContentSubmissionItems')) return;
    setError('');
    try {
      const previews = [];
      for (const { batch } of cleanableBatches) {
        const preview = await commands.previewCleanupFailedContentSubmissionItems({ batchId: batch.id });
      ...
      try {
        const results = []; for (const preview of previews) if (preview.cleanableCount) {
          const result = await commands.cleanupFailedContentSubmissionItems({ batchId: preview.batchId });
          if (isContentCommandStaleResult(result)) return;
          results.push(result);
        }
  ...
  async function previewTrashSelections(selections: Array<{ clientId: string; articleId: string }>) {
    ...
    setError(''); setTrashFeedback(null);
    try {
      ...
      if (existingTransactionId) await watchRemovalTransaction(existingTransactionId);
  ...
  async function commitTrash(previewOverride?: ArticleTrashPreview) {
    ...
    try {
      ...
      if (resultTransactionId) await watchRemovalTransaction(resultTransactionId);
      setTrashPreview(null);
      updateSelected([]);
  ...
  async function retryRemovalTransaction() {
    ...
    try {
      ...
      if (isContentCommandStaleResult(next) || !isCurrentClient(requestedClientId)) return;
  ...
  async function restoreOne(entry: ArticleTrashRecord) {
    ...
    setError(''); try {
      const result = await commands.restoreContentArticle({ clientId: entry.clientId, articleId: entry.articleId });
      if (isContentCommandStaleResult(result) || !isCurrentClient(requestedClientId)) return;
  ...
  async function permanentlyDeleteOne(entry: ArticleTrashRecord) {
    const requestedClientId = clientId;
    if (commandBusy('preparePermanentDeleteContentArticle', 'permanentlyDeleteContentArticle')) return;
    setError('');
    let prepared;
    try {
    ...
    setError(''); try {
      const result = await commands.permanentlyDeleteContentArticle({ clientId: entry.clientId, articleId: entry.articleId, token: prepared.token });
      if (isContentCommandStaleResult(result) || !isCurrentClient(requestedClientId)) return;
  ...
  if (selectedStage === 'trash') return <ArticleTrashPanel
    trash={trash}
    visibleError={visibleError}
    commandBusy={commandBusy}
    onBack={() => { const next = lastNonTrashStageRef.current; setSelectedStage(next); onStageFilterChange?.(next); }}
  ...
  return <div className="relative h-full w-full min-w-0 overflow-y-auto p-4">
    <div className="mb-4 grid min-w-0 gap-3">
      ...
      <div className="grid min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr)]">
       ...
       <div className="flex min-w-0 flex-wrap items-center gap-2">
         ...
         <button type="button" onClick={() => void trashSelected()} disabled={!selectedArticles.length || commandBusy('previewContentArticleRemoval', 'trashContentArticles') || removalSubmitDisabled} className="rounded bg-rose-600 px-3 py-2 text-xs font-semibold text-white disabled:opacity-40">移入回收站 ({selectedArticles.length})</button>
       ...
       <div className="flex min-w-0 flex-wrap items-start gap-2 rounded-md border border-slate-200 bg-slate-50 p-2">
        ...
        <button type="button" onClick={() => void handoffSelectedToMedia()} disabled={!selectedArticles.some((article) => article.status === 'generated' || article.status === 'saved') || commandBusy('previewExport', 'exportToSubmissionQueue')} className="shrink-0 rounded border border-blue-300 bg-white px-3 py-2 text-xs font-semibold text-blue-700 disabled:opacity-40">加入付费媒体投稿</button>
    ...
    <GeneratedArticlesList groups={groups} visibleError={visibleError} clientId={clientId} collapsed={collapsed} selected={selected} queuedArticleIds={queuedArticleIds} publicationRecordsByArticle={publicationRecordsByArticle} publicationSummaries={publicationSummaries} workflowByArticle={workflowByArticle} removalSubmitDisabled={removalSubmitDisabled} commandBusy={commandBusy} onToggleCollapsed={(key) => setCollapsed((current) => ({ ...current, [key]: current[key] === false }))} onToggleGroup={toggleGroup} onToggleArticle={toggleArticle} onOpenArticle={openArticle} onOpenPublication={(article) => setDrawerArticle(article)} onTrashPublishedArticle={(article) => void trashPublishedArticle(article)} />
