...
import { commitGenerationSubmissionHandoff, listContentSubmissionPlatforms, previewGenerationSubmissionHandoff } from '../../electron-api';
...
export default function GenerationSubmissionHandoffDrawer({ batch, onClose, onCommitted, onOpenOtherPlatform }: GenerationSubmissionHandoffDrawerProps) {
  ...
  async function inspect() {
    ...
    try { setPreview(await previewGenerationSubmissionHandoff({ generationBatchId: batch.id, targetPlatformIds })); }
  ...
  async function commit() {
    ...
    try {
      const next = await commitGenerationSubmissionHandoff({ generationBatchId: batch.id, targetPlatformIds, previewToken: preview.previewToken, confirmed: true });
      setResult(next);
      onCommitted?.(next);
  ...
  return <div className="fixed inset-0 z-40 flex justify-end bg-slate-900/30" role="dialog" aria-modal="true" aria-labelledby="generation-submission-handoff-title">
    <section className="flex h-full w-full max-w-2xl flex-col overflow-hidden bg-white shadow-xl">
      <div className="flex shrink-0 items-start gap-3 border-b border-slate-200 px-5 py-4"><div className="min-w-0 flex-1"><h2 id="generation-submission-handoff-title" className="text-base font-semibold text-slate-800">生成批次投稿交接</h2><p className="mt-1 text-xs leading-5 text-slate-500">只创建本地投稿队列，不自动执行远端发布。</p></div><button type="button" onClick={onClose} disabled={busy} aria-label="关闭生成批次投稿交接" className="rounded px-2 py-1 text-lg text-slate-400 hover:bg-slate-100">×</button></div>
