...
export default function GenerationSubmissionHandoffDrawer({ batch, onClose, onCommitted, onOpenOtherPlatform }: GenerationSubmissionHandoffDrawerProps) {
  ...
  async function inspect() {
    if (!platformId) { setError('请先选择投稿目标'); return; }
    if (!accountProfileId) { setError('请先选择平台账号档案'); return; }
    setBusy(true); setError(''); setResult(null);
    try { setPreview(await previewSubmissionHandoff({ generationBatchId: batch.id, platformId, accountProfileId })); }
  ...
  async function commit() {
    ...
    try {
      const next = await commitSubmissionHandoff({ generationBatchId: batch.id, platformId, accountProfileId, previewToken: preview.previewToken, confirmed: true });
      setResult(next);
      onCommitted?.(next);
  ...
  return <div className="fixed inset-0 z-[100] flex justify-end bg-slate-900/30" role="dialog" aria-modal="true" aria-labelledby="generation-submission-handoff-title">
    <section className="flex h-full w-full max-w-2xl flex-col overflow-hidden bg-white shadow-xl">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-5">
        {!result && <>
          <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4"><div className="rounded bg-slate-50 p-3">成功文章：<strong>{successCount}</strong></div><div className="rounded bg-slate-50 p-3">涉及客户：<strong>{clientCount}</strong></div><div className="rounded bg-slate-50 p-3">投稿目标：<strong>{platformId ? 1 : 0}</strong></div><div className="rounded bg-slate-50 p-3">队列任务：<strong>{platformId ? successCount : 0}</strong></div></div>
          <div className="mt-5 rounded border border-slate-200 p-3"><h3 className="text-sm font-semibold">选择投稿目标</h3><div className="mt-3"><select aria-label="生成批次投稿目标" value={platformId} onChange={(event) => { setPlatformId(event.target.value); setAccountProfileId(''); setPreview(null); setResult(null); }} className="h-9 w-full rounded border border-slate-300 px-2 text-xs"><option value="">请选择一个投稿目标</option>{platforms.map((platform) => <option key={platform.id} value={platform.id}>{platform.displayName || platform.id}</option>)}</select>{!platforms.length && <p className="mt-2 text-xs text-slate-500">没有可用的投稿目标。</p>}</div></div>
          <div className="mt-3"><AccountProfileSelector platforms={platforms} platformId={platformId} value={accountProfileId} onChange={(next) => { setAccountProfileId(next); setPreview(null); }} /></div>
