...
import { LogIn, Pause, Play, RotateCcw, Square } from 'lucide-react';
...
export default function CollectionTaskBar({ queue, busy = false, onStart, onPause, onResume, onStop, onRetry, onLogin }: CollectionTaskBarProps) {
  ...
  return <div style={{ width: '100%', height: 56, minHeight: 56, maxHeight: 56 }} className="collection-task-bar h-14 min-h-14 max-h-14 shrink-0 overflow-hidden border-t border-slate-200 bg-white px-3 flex items-center justify-between gap-3">
    ...
    <div className="flex items-center gap-1">
      {!active && <button type="button" disabled={busy || active} onClick={onStart} title="开始批量采集" className="task-icon-button"><Play className="h-4 w-4" /></button>}
      ...
      <button type="button" onClick={onLogin} title="打开豆包登录" className="task-icon-button"><LogIn className="h-4 w-4" /></button>
    </div>
  </div>;
}
