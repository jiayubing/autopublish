...
export default function QuestionResearchList({ questions, research, manualAnswerSession, manualSourceRef, isCollecting, onToggle, onEdit, onManualAnswer, onCollect, onRecollect, onDelete, focusQuestionId }: QuestionResearchListProps) {
  ...
  return <div className="space-y-2">{questions.map((question) => {
    ...
    return <div key={question.id}><CollapsibleSourceItem id={question.id} title={question.text} summary={summary} selected={question.enabled} onSelectedChange={() => onToggle(question)} defaultExpanded={question.id === focusQuestionId} actions={actions}>
