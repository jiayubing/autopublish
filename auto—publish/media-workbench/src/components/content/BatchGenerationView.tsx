...
import { BATCH_GENERATION_STEPS, countGenerationTasks, getMaterialId, groupTemplatesByPlatform, isUsableMaterial, isUsableResearch, sourceCharacterCount } from '../../content-generation-ui-logic';
...
function materialForClient(client: ContentClient): ContentMaterial[] {
...
export default function BatchGenerationView({ clients, refreshToken, onRefresh }: BatchGenerationViewProps) {
  ...
  const currentSources = useMemo<GenerationBatchSourceSelection[]>(() => selectedClientIds.map((clientId) => ({
    clientId,
    materialIds: sources[clientId]?.materialIds || [],
    researchQueryIds: sources[clientId]?.researchQueryIds || [],
  })), [selectedClientIds, sources]);
  ...
  const executableClients = selectedClientIds.filter((clientId) => {
    ...
    return Boolean(source?.materialIds.some((id) => isUsableMaterial(materials.find((item) => getMaterialId(item) === id))) && source?.researchQueryIds.some((id) => isUsableResearch(research.find((item) => item.id === id))));
  });
  const executableTaskCount = previewResult?.executableTaskCount ?? executableClients.length * selectedTemplates.length;

  useEffect(() => {
    setSelectedClientIds((current) => current.length ? current.filter((id) => clientMap.has(id)) : clients.map((client) => client.id));
  }, [clients, clientMap, refreshToken]);

  useEffect(() => {
    let cancelled = false;
    Promise.all(PLATFORMS.map(async (platform) => {
      const { listContentTemplates } = await import('../../electron-api');
      return listContentTemplates(platform);
    })).then((groups) => {
      if (cancelled) return;
      const nextTemplates = groups.flat();
      setTemplates(nextTemplates);
      setSelectedTemplates(nextTemplates.map((item) => ({ platform: item.platform, templateId: item.id })));
  ...
  return <div className="batch-generation-view flex h-full min-h-0 flex-col overflow-hidden" aria-label="四步批量生成">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto p-4">
      {step === 0 && <section className="rounded-md border border-slate-200 bg-white p-4"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><h2 className="text-sm font-semibold">选择批次客户</h2><p className="mt-1 text-xs text-slate-500">已选 {selectedCount} 个客户</p></div><div className="flex gap-2"><button type="button" onClick={toggleAllClients} className="rounded border border-slate-300 px-3 py-2 text-xs">全选客户</button><button type="button" onClick={() => { setSelectedClientIds([]); setPreviewResult(null); }} disabled={!selectedCount} className="rounded border border-slate-300 px-3 py-2 text-xs disabled:opacity-40">取消全选</button></div></div><label className="mb-3 flex items-center gap-2 text-xs font-semibold"><input type="checkbox" checked={allSelected} onChange={toggleAllClients} />全选客户</label><div className="grid gap-2 sm:grid-cols-2">{clients.map((client) => <label key={client.id} className="flex items-center gap-2 rounded border border-slate-200 p-3 text-sm"><input type="checkbox" checked={selectedClientIds.includes(client.id)} onChange={(event) => setSelectedClientIds((current) => event.target.checked ? [...new Set([...current, client.id])] : current.filter((id) => id !== client.id))} />{client.name}</label>)}</div></section>}
      {step === 1 && <section className="rounded-md border border-slate-200 bg-white p-4"><h2 className="text-sm font-semibold">选择跨平台写作模板</h2><p className="mt-1 text-xs text-slate-500">模板按平台分组，已选 {selectedTemplates.length} 个</p><div className="mt-4 grid gap-4 md:grid-cols-3">{PLATFORMS.map((platform) => <div key={platform} className="rounded border border-slate-200 p-3"><h3 className="text-xs font-semibold uppercase text-slate-700">{platform}</h3><div className="mt-2 grid gap-2">{(templateGroups[platform] || []).map((template) => <label key={template.id} className="flex items-start gap-2 text-xs text-slate-600"><input type="checkbox" checked={selectedTemplates.some((item) => item.platform === platform && item.templateId === template.id)} onChange={() => toggleTemplate(template)} /><span><span className="block font-medium">{template.name || template.scenario}</span><span className="text-slate-400">{template.scenario}</span></span></label>)}</div></div>)}</div></section>}
      {step === 2 && <section className="grid gap-3">{selectedClientIds.map((clientId) => { const client = clientMap.get(clientId); const materials = materialForClient(client || { id: clientId, name: clientId, knowledgeFiles: [] }); const research = researchByClient[clientId] || []; const source = sources[clientId] || { materialIds: [], researchQueryIds: [] }; const selectedMaterials = materials.filter((item) => source.materialIds.includes(getMaterialId(item)) && isUsableMaterial(item)); const selectedResearch = research.filter((item) => source.researchQueryIds.includes(item.id) && isUsableResearch(item)); const reason = !selectedMaterials.length ? '没有有效客户资料' : !selectedResearch.length ? '没有有效 GEO 调研回答' : ''; return <article key={clientId} className="rounded-md border border-slate-200 bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><h2 className="text-sm font-semibold">{client?.name || clientId}</h2><p className="mt-1 text-xs text-slate-500">预计输入字符数 {sourceCharacterCount(selectedMaterials, selectedResearch)}</p></div><span className={`text-xs font-semibold ${reason ? 'text-rose-600' : 'text-emerald-600'}`}>{reason || '可生成'}</span></div><div className="mt-3 grid gap-2">{materials.map((item) => <CollapsibleSourceItem key={getMaterialId(item)} id={`${clientId}-${getMaterialId(item)}`} title={item.name} summary={`${item.extension || '资料'} · ${item.characterCount || 0} 字${item.status === 'error' ? ' · 错误' : ''}`} selected={source.materialIds.includes(getMaterialId(item))} onSelectedChange={(selected) => updateSource(clientId, 'materialIds', getMaterialId(item), selected)} defaultExpanded={false}>{item.content || '资料转换失败，请重试。'}</CollapsibleSourceItem>)}{research.map((item) => <CollapsibleSourceItem key={item.id} id={`${clientId}-${item.id}`} title={item.question || item.id} summary={`${item.answerText?.length || 0} 字 · GEO 调研回答${item.isAnswerComplete === false ? ' · 未完成' : ''}`} selected={source.researchQueryIds.includes(item.id)} onSelectedChange={(selected) => updateSource(clientId, 'researchQueryIds', item.id, selected)} defaultExpanded={false}>{item.answerText || '没有回答内容'}</CollapsibleSourceItem>)}</div></article>; })}</section>}
