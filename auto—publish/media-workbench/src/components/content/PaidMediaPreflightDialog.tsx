...
export default function PaidMediaPreflightDialog({
  model,
  busy,
  error,
  onClose,
  onConfirm,
}: PaidMediaPreflightDialogProps) {
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="paid-media-preflight-title"
        tabIndex={-1}
      ...
      >
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5 text-sm text-slate-700">
          <div className="grid gap-2 sm:grid-cols-2">
            <div className="rounded border border-slate-200 bg-slate-50 p-3">
              <div className="text-xs text-slate-500">媒体资源</div>
              <div className="mt-1 font-semibold">
                {model.mediaName || model.mediaResourceId}
              </div>
              <div className="mt-1 text-xs text-slate-500">
                {model.mediaResourceId}
              </div>
            </div>
            <div className="rounded border border-slate-200 bg-slate-50 p-3">
              <div className="text-xs text-slate-500">媒体备注</div>
              <div className="mt-1 whitespace-pre-wrap text-xs leading-5">
                {model.mediaRemarks || "无"}
              </div>
            </div>
            <div className="rounded border border-slate-200 bg-slate-50 p-3">
            ...
            <div className="rounded border border-slate-200 bg-slate-50 p-3">
              ...
              <div className="mt-1 font-semibold">
                {model.quotedPrice === null
                  ? "报价无效"
                  : `¥${model.quotedPrice.toFixed(2)}`}{" "}
                /{" "}
                {model.estimatedTotal === null
                  ? "无法计算"
                  : `¥${model.estimatedTotal.toFixed(2)}`}
              </div>
            </div>
          </div>

          <div className="rounded border border-blue-100 bg-blue-50 p-3 text-xs text-blue-800">
            <span className="font-semibold">系统投稿标识码：</span>
            {model.systemSubmissionCode || "未配置"}
          </div>

          <section>
            <h3 className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-slate-600">
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-slate-500">
                      {article.articleRef.clientId} · {article.articleId}
                    </div>
                  </div>
                  <span
                    className={
                  ...
                  >
                    {article.status === "ready"
                      ? "可确认"
                      : `阻断：${article.reasonCodes.join("、") || "状态不允许"}`}
                  </span>
                </div>
              ))}
            </div>
          ...
          {model.blockers.length > 0 && (
            <div
              role="alert"
              className="rounded border border-rose-200 bg-rose-50 p-3 text-xs text-rose-700"
            >
        ...
        <div className="flex justify-end gap-2 border-t border-slate-100 bg-slate-50/70 p-4">
          <button
            ...
            onClick={onClose}
            disabled={busy}
            className="rounded border border-slate-300 bg-white px-3 py-2 text-xs disabled:opacity-40"
          >
            取消
          </button>
          <button
            ...
            disabled={busy || model.canConfirm !== true}
            className="flex items-center gap-1.5 rounded bg-blue-600 px-3 py-2 text-xs font-semibold text-white disabled:opacity-40"
          >
