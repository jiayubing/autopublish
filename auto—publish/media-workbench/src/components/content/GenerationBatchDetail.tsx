...
import { cancelPendingGenerationBatch, previewCancelPendingGenerationBatch } from '../../bridge/content';
...
export default function GenerationBatchDetail({ batch, state, busy, onPause, onResume, onContinue, onStop, onRetry, onRefreshArticles, onStartNew }: GenerationBatchDetailProps) {
  ...
  const [handoffSummary, setHandoffSummary] = useState<GenerationSubmissionHandoffResult | null>(null);
  ...
  const active = effectiveStatus === 'running' || effectiveStatus === 'pausing' || effectiveStatus === 'stopping';
  ...
  const showCostWarning = active || (['paused', 'stopped'].includes(batch.status) && unfinished);
  const failed = counts.failed > 0;
  const terminal = effectiveStatus === 'completed' || effectiveStatus === 'stopped';
  ...
  return <section className="generation-batch-detail mt-4 rounded-md border border-slate-200 bg-white p-4" aria-live="polite">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-sm font-semibold">批量生成进度</h2><p className="mt-1 text-xs text-slate-500">批次 {displayedBatch.id} · 状态 {effectiveStatus}</p></div><div className="flex gap-1"><button type="button" title="暂停批量生成" onClick={onPause} disabled={busy || !running} className="task-icon-button"><Pause className="h-4 w-4" /></button><button type="button" title="继续批量生成" onClick={onResume} disabled={busy || active || !unfinished} className="task-icon-button"><Play className="h-4 w-4" /></button><button type="button" title="停止批量生成" onClick={onStop} disabled={busy || !running} className="task-icon-button"><Square className="h-4 w-4" /></button><button type="button" title="重试失败任务" onClick={onRetry} disabled={busy || active || !failed} className="task-icon-button"><RotateCcw className="h-4 w-4" /></button><button type="button" title="永久取消待处理任务" onClick={() => void cancelPending()} disabled={busy || cancelBusy || counts.pending < 1} className="task-icon-button"><Ban className="h-4 w-4" /></button></div></div>
    ...
    {canContinue && <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded border border-amber-100 bg-amber-50 p-2 text-xs text-amber-800"><span>批次尚有未完成任务，确认后继续执行。</span><button type="button" onClick={onContinue} disabled={busy} className="rounded bg-amber-700 px-2 py-1 text-white">继续未完成</button></div>}
    ...
    {handoffOpen && <GenerationSubmissionHandoffDrawer batch={displayedBatch} onClose={() => setHandoffOpen(false)} onCommitted={(result) => { onRefreshArticles?.(); if (!result.failedClientGroups.length) { setHandoffSummary(result); setHandoffOpen(false); } }} />}
  </section>;
}
