...
export default function GenerationBatchDetail({ batch, state, busy, onPause, onResume, onContinue, onStop, onRetry }: GenerationBatchDetailProps) {
  ...
  return <section className="generation-batch-detail mt-4 rounded-md border border-slate-200 bg-white p-4" aria-live="polite">
    ...
    <div className="generation-batch-task-list mt-4 max-h-56 space-y-1 overflow-y-auto">{batch.tasks.map((task) => <div key={task.id} className="flex flex-wrap items-center justify-between gap-2 rounded border border-slate-100 px-2 py-2 text-xs"><span className="min-w-0 truncate">{task.clientId} · {task.platform} · {task.templateId}</span><span className={task.status === 'failed' ? 'text-rose-600' : task.status === 'succeeded' ? 'text-emerald-600' : 'text-slate-500'}>{task.status}{task.error?.message ? ` · ${task.error.message}` : ''}</span></div>)}</div>
  </section>;
}
