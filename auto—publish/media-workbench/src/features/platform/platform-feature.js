...
export function remainingPreviewLabel(group) {
  const remaining = Array.isArray(group && group.remaining) ? group.remaining : [];
  const previewCount = remaining.length;
  const remainingCount = Number.isInteger(group && group.remainingCount)
    ? group.remainingCount
    : previewCount;
  if (remainingCount > previewCount)
    return `当前展示前 ${previewCount} 条，共 ${remainingCount} 条待执行`;
  return `待执行 · ${remainingCount}`;
}
