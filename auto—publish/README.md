...
文章库列表只加载摘要，打开编辑器后再加载完整正文。关键词搜索仍支持正文和模板正文，搜索失败可重试；迟到的旧搜索结果不会覆盖当前筛选。
...
发布档案列表不再携带投稿正文；打开“发布详情”后读取完整不可变档案，失败可重试。文章摘要列表和全文搜索分段执行，期间可响应其他主进程任务；同客户的新搜索会结束尚未完成的旧搜索。
