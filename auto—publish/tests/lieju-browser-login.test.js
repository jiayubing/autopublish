...
const evaluator = `
  ...
  global.window = { location: new URL(fixture.url) };
  ...
  const page = {
    ...
    evaluate: async fn => new Function('window', 'document', 'URL', 'return (' + fn.toString() + ')()')(window, document, URL)
  };
  const AsyncFunction = Object.getPrototypeOf(async function(){}).constructor;
  new AsyncFunction('page', 'URL', code)(page, undefined).then(value => process.stdout.write(JSON.stringify(value))).catch(error => process.stdout.write(JSON.stringify('### Error\\n' + error.message)));
`;
...
mock.method(playwright, "runCode", (code) => {
  ...
  if (current.runtimeResult !== undefined) return current.runtimeResult;
  return JSON.parse(
    execFileSync(
      process.execPath,
      ["-e", evaluator, JSON.stringify({ code, fixture: current })],
...
function check(account, home) {
  fixtures = {
    [LIEJU.accountUrl]: account,
    [LIEJU.base]: home || { url: LIEJU.base, html: "" },
  };
  navigations = [];
...
test("accepts a visible logout link or an account-page profile link", () => {
  assert.equal(
    check({ url: LIEJU.accountUrl, html: '<a href="?action=quit">退出</a>' }),
    true,
  );
  assert.deepEqual(navigations, [LIEJU.accountUrl]);
  assert.equal(
    check({ url: LIEJU.accountUrl, html: '<a href="/u12345">合成账号</a>' }),
    true,
  );
  assert.equal(
    check({
      url: LIEJU.base,
      html: '<a hidden href="?action=quit">退出</a><a href="?action=quit">退出</a>',
    }),
    true,
  );
});
...
test("does not accept a foreign page or hidden logout markup", () => {
  assert.equal(
    check({
      url: "https://example.test/member/upage.php",
      html: '<a href="?action=quit">退出</a>',
    }),
    false,
  );
  assert.equal(
    check({
      url: LIEJU.accountUrl,
      html: '<a hidden href="?action=quit">退出</a>',
    }),
    false,
  );
});

test("CLI error text and unexpected results are never login evidence", () => {
...
test("does not treat public user links or external profile links as account evidence", () => {
  assert.equal(
    check({ url: LIEJU.base, html: '<a href="/u12345">公开用户</a>' }),
    false,
  );
  assert.equal(
    check({
      url: LIEJU.accountUrl,
      html: '<a href="https://example.test/u12345">外部用户</a>',
    }),
    false,
  );
});
...
test("falls back to home when the account page cannot be opened", () => {
  assert.equal(
    check(
      ...
      { url: LIEJU.base, html: '<a href="?action=quit">退出</a>' },
    ),
    true,
  );
