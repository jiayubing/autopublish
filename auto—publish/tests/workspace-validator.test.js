...
function writeMarker(directory, value) {
  fs.writeFileSync(path.join(directory, ".autopublish-workspace.json"), JSON.stringify(value), "utf8");
}
...
function createValidator(root, overrides) {
  ...
  return createWorkspaceValidator(Object.assign({
    appPath,
    resourcesPath,
    userDataPath,
...
describe("workspace validator", function() {
  ...
  it("classifies a valid version 1 marker as an existing workspace", function() {
    ...
    writeMarker(workspace, { version: 1, createdAt: "2026-07-14T00:00:00.000Z" });
    try {
  ...
  it("rejects a directory when the random write probe cannot create a file", function() {
    ...
    try {
      ...
      const result = createValidator(root, { fs: blockedFs }).validate(candidate);
      assert.equal(result.kind, "invalid");
  ...
  it("rejects damaged, unknown-version, and linked markers", function(t) {
    ...
    try {
      for (const content of [
        ...
        JSON.stringify({ version: 1, createdAt: "2026-07-14T00:00:00.000Z", extra: true })
      ]) {
      ...
      fs.writeFileSync(linkedMarkerTarget, JSON.stringify({ version: 1, createdAt: "2026-07-14T00:00:00.000Z" }), "utf8");
      try {
