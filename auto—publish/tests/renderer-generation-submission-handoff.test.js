...
describe("renderer generation submission handoff", { concurrency: false }, function() {
  ...
  it("closes the modal after a successful handoff and leaves a non-modal summary", async function() {
    ...
    await page.addInitScript(() => {
      ...
      const content = {
        listClients: () => ok([client]), listResearch: () => ok([]), listQuestions: () => ok([]), listGeneratedArticles: () => ok([]), listSubmissionBatches: () => ok([]), listArticleTrash: () => ok([]), getDoubaoLoginState: () => ok({ status: "unknown" }), getDoubaoQueueState: () => ok({ status: "idle", currentTaskId: null, completed: 0, total: 0, waitRemainingMs: 0, tasks: [] }), onDoubaoQueueState: () => () => {},
        ...
        listGenerationBatches: () => ok([batch]), getGenerationBatch: () => ok(batch), getGenerationBatchState: () => ok({ status: "idle", state: "idle", batchId: null }), onGenerationBatchState: () => () => {},
