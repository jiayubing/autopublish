...
describe('Phase 06 attention feature', () => {
  it('lets the workspace coordinator own the single production initial refresh', () => {
    const source = fs.readFileSync(
      path.resolve(import.meta.dirname, '../media-workbench/src/features/attention/use-attention-feature.ts'),
      'utf8',
    );
  ...
  it('owns the scoped query revision/fingerprint and rejects an older response', async () => {
    ...
    newQuery.resolve({ revision: 8, items: [attention('new')], counts: { total: 1, actionable: 1 } });
    await invalidation;
    oldQuery.resolve({ revision: 7, items: [attention('old')], counts: { total: 1, actionable: 1 } });
    await initial;

    const snapshot = feature.getSnapshot();
    assert.deepEqual(snapshot.scope, { workspaceRuntimeId: 'workspace-1', clientId: 'client-1' });
    ...
    assert.deepEqual(snapshot.query, { loading: false, error: null, reason: 'invalidation' });
  });
  ...
  it('keeps preview and execute lifecycles independent and binds execute to the query fingerprint', async () => {
    ...
    let revision = 3;
    const feature = createAttentionFeature({
      list: async () => ({ revision, items: [attention('attention-1', ['cleanup'])], counts: { total: 1, actionable: 1 } }),
      preview: () => previewRequest.promise,
      execute: (input) => { executeInputs.push(input); return executeRequest.promise; },
    });
    feature.setScope({ workspaceRuntimeId: 'workspace-1', clientId: 'client-1' });
    await feature.refresh('initial');
    const queryFingerprint = feature.getSnapshot().fingerprint;

    const previewing = feature.previewAction({ attentionId: 'attention-1', action: 'cleanup' });
    assert.equal(feature.getSnapshot().commands.preview.busy, true);
    assert.equal(feature.getSnapshot().commands.execute.busy, false);
    previewRequest.resolve({ attentionId: 'attention-1', revision: 3, action: 'cleanup', requiresConfirmation: true, message: '将清理队列残留', changedScopes: [] });
    const preview = await previewing;
    assert.equal(feature.getSnapshot().commands.preview.busy, false);
    assert.equal(feature.getSnapshot().pendingPreview.bindingFingerprint, queryFingerprint);

    const executing = feature.executePreview(preview, { confirmed: true });
    assert.equal(feature.getSnapshot().commands.preview.busy, false);
    assert.equal(feature.getSnapshot().commands.execute.busy, true);
    assert.deepEqual(executeInputs, [{ attentionId: 'attention-1', action: 'cleanup', expectedRevision: 3, confirmed: true }]);
    revision = 4;
    executeRequest.resolve({ outcome: 'resolved', attentionId: 'attention-1', changedScopes: ['articleAttention'] });
    await executing;
    assert.equal(feature.getSnapshot().commands.execute.busy, false);
  ...
  it('rejects execute when a newer query invalidates the preview fingerprint', async () => {
    ...
    const feature = createAttentionFeature({
      list: async () => ({ revision, items: [attention('attention-1', ['cleanup'])], counts: { total: 1, actionable: 1 } }),
      preview: async () => ({ attentionId: 'attention-1', revision: 10, action: 'cleanup', requiresConfirmation: true, message: '确认', changedScopes: [] }),
      execute: async () => { executeCalls += 1; return {}; },
    });
    feature.setScope({ workspaceRuntimeId: 'workspace-1', clientId: 'client-1' });
    await feature.refresh('initial');
    const preview = await feature.previewAction({ attentionId: 'attention-1', action: 'cleanup' });
    revision = 11;
    await feature.refresh('invalidation');

    await assert.rejects(feature.executePreview(preview, { confirmed: true }), { code: 'ARTICLE_ATTENTION_STALE' });
    assert.equal(executeCalls, 0);
    assert.equal(feature.getSnapshot().commands.execute.busy, false);
    assert.equal(feature.getSnapshot().commands.execute.error.code, 'ARTICLE_ATTENTION_STALE');
  });
});
