...
describe("media-resource-service", function () {
  ...
  it("pages cached resources with metadata", function () {
    ...
    const paged = createMediaResourceService({
      resourceStore: store,
  ...
  it("canonicalizes only finite non-negative supplier money and never turns invalid values into zero", async function () {
    const written = [];
    const result = await createMediaResourceService({
      ...
      client: {
        mediaList: async function () {
          return {
            data: [
              { resource_id: "valid", price: "12.5" },
              { resource_id: "negative", price: "-10" },
              { resource_id: "currency", price: "¥12" },
              { resource_id: "nan", price: "NaN" },
              { resource_id: "infinite", price: Infinity },
              { resource_id: "huge", price: "100000001" },
            ],
            total: 6,
            hasNext: false,
          };
    ...
    assert.equal(
      result.diagnostics.find(
        (item) => item.code === "MEDIA_RESOURCE_PRICE_INVALID",
      ).count,
      5,
    );
    assert.deepEqual(
      written[0].resources.map((item) => [item.resourceId, item.price]),
      [
        ["valid", 12.5],
        ["negative", undefined],
        ["currency", undefined],
    ...
    assert.equal(
      written[0].resources.some((item) => "raw" in item),
      false,
    );
    assert.doesNotMatch(
      JSON.stringify(written[0].resources),
      /-10|¥12|NaN|100000001/,
    );
    assert.throws(
      () =>
        createMediaResourceService().addToPool({
          resourceId: "negative",
          price: "-10",
        }),
      { code: "MEDIA_RESOURCE_PRICE_INVALID" },
    );
  });
  ...
  it("rejects page sizes above 100 and renderer-controlled refresh limits", async function () {
    ...
    assert.throws(
      function () {
        bounded.getCachedResourcePage({ page: 1, pageSize: 101 });
      },
      function (error) {
        return error.code === "MEDIA_RESOURCE_PAGE_SIZE_INVALID";
      },
    );
    assert.throws(
      function () {
        bounded.searchResourcePage({ keyword: "x", page: 1, pageSize: 99999 });
      },
      function (error) {
        return error.code === "MEDIA_RESOURCE_PAGE_SIZE_INVALID";
      },
    ...
    await assert.rejects(
      ...
      function (error) {
        return error.code === "MEDIA_RESOURCE_REFRESH_OPTIONS_INVALID";
      },
    );
  });
  ...
  it("refreshes all pages until the api returns a short page and writes the cache", async function () {
    ...
    assert.deepStrictEqual(
      written[0].resources.map(function (resource) {
  ...
  it("keeps 1k, 10k, 13k, and 20k main refreshes bounded and measurable", async function (t) {
    for (const total of [1000, 10000, 13000, 20000]) {
      let requestCount = 0;
      let cached = null;
      ...
      const result = await createMediaResourceService({
        resourceStore: {
          ...
          setAll: function (resources, meta) {
            cached = { resources: resources, meta: meta };
          },
        },
        client: {
          mediaList: async function (input) {
            ...
            return {
              data: Array.from(
                ...
                function (_, index) {
                  return {
                    resource_id: String(start + index + 1),
                    title: "Synthetic " + (start + index + 1),
                  };
                },
              ),
              total: total,
      ...
      const mainHeapDeltaBytes = Math.max(
        0,
        process.memoryUsage().heapUsed - beforeHeap,
      );
      const payloadBytes = Buffer.byteLength(JSON.stringify(cached));

      assert.equal(requestCount, total / 100);
      assert.equal(result.status, "complete");
      assert.equal(result.complete, true);
      assert.equal(result.truncated, false);
      assert.equal(result.resourceCount, total);
      assert.equal(cached.resources.length, total);
      t.diagnostic(
        JSON.stringify({
          fixture: "synthetic-main-refresh",
          total,
          requestCount,
          payloadBytes,
          mainHeapDeltaBytes,
        ...
        mediaList: async function (input) {
          ...
          return {
            data: Array.from({ length: 20 }, function (_, index) {
              return { id: String(start + index + 1) };
            }),
          };
        },
      },
  ...
  it("deduplicates by resource ID and reports a repeated remote page as truncated", async function () {
    ...
    assert.deepEqual(
      result.diagnostics.map(function (item) {
        return item.code;
      }),
      ["MEDIA_RESOURCE_DUPLICATE_IDS", "MEDIA_RESOURCE_REPEATED_PAGE"],
    );
    assert.deepEqual(
      createMediaResourceService({ resourceStore: resourceStore })
        .getCachedResourcePage({ page: 1, pageSize: 100 })
  ...
  it("truncates explicitly at the 20,001st unique resource without returning the full cache payload", async function () {
    ...
    const serviceWithCapacity = createMediaResourceService({
      ...
      client: {
        mediaList: async function () {
          return { data: oversizedPage };
        },
      },
    });

    const result = await serviceWithCapacity.refreshResources({
      fetchAll: true,
      pageSizeHint: 100,
    });
    ...
    assert.equal(
      ...
      "MEDIA_RESOURCE_MAX_RESOURCES_REACHED",
    );
  ...
  it("stops after 200 full remote pages and marks the cache as truncated", async function () {
    ...
    const result = await createMediaResourceService({
      ...
      client: {
        mediaList: async function (input) {
          ...
          const start = (input.page - 1) * 100;
          return {
            data: Array.from({ length: 100 }, function (_, index) {
              return { id: String(start + index + 1) };
            }),
          };
        },
    ...
    assert.equal(
      result.diagnostics.at(-1).code,
      "MEDIA_RESOURCE_MAX_PAGES_REACHED",
    );
    assert.equal(cached.meta.truncated, true);
  });
  ...
  it("adds a normalized resource and returns a bounded pool page", function () {
    ...
    var page = serviceWithRealStores.getPoolPage({
      page: 1,
      pageSize: 50,
      resourceIds: ["501"],
    });
