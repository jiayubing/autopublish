    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          'registerAiContentIpc({ ipcMain: createAuthenticatedIpcMain(ipcMain, async () => {}), aiContentService: { generateArticle: async () => ({ id: "article-1", clientId: "client-1", researchQueryIds: ["research-1"], researchSnapshots: [{ questionId: "research-1", answerText: "回答", references: [], collectedAt: undefined, collectionMethod: undefined }], platform: "platform-1", scenario: "介绍", templateId: "template-1", title: "标题", content: "正文", status: "generated", source: { client_material: true, doubao_answer: true, references: false, template: true }, createdAt: "2026-07-27T00:00:00.000Z", reviewedAt: null }) } });',
          'registerArticleManagementIpc({ ipcMain: createAuthenticatedIpcMain(ipcMain, async () => {}), articleManagementSnapshot: { get: () => ({ clientId: "畅途", revision: 1, articles: [{ id: "article-reference", clientId: "畅途", researchQueryIds: ["调研问题"], researchSnapshots: [{ questionId: "调研问题", answerText: "调研回答", references: [{ title: "引用", url: "https://example.com", snippet: "摘".repeat(10001) }], collectionMethod: "automatic" }], platform: "微信公众号", scenario: "介绍", templateId: "新闻稿", title: "标题", content: "正文", status: "saved", source: { client_material: true, doubao_answer: true, references: true, template: true }, createdAt: "2026-07-27T00:00:00.000Z", reviewedAt: null }], trash: [], submissionBatches: [], cancellationPlans: [], publicationRecords: [], attention: { revision: 1, items: [], counts: { total: 0, actionable: 0 } }, submissionPlatforms: [], workflowByArticle: {}, publicationSummaries: {} }) } });',
      ...
      const exposure = await page.evaluate(async () => ({
        ...
        managementReference: await (async () => {
          ...
          return { ok: result?.ok, length: result?.data?.articles?.[0]?.researchSnapshots?.[0]?.references?.[0]?.snippet?.length, errorCode: result?.error?.code || null };
        })(),
