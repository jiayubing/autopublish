...
describe("Phase 08 platform/media/settings/workspace renderer slice", () => {
  ...
  it("coordinates queue-group commands through the renderer feature without overriding manual pause", async () => {
    ...
    const feature = createPlatformFeature({
      ...
      pauseAllRegularQueueGroups: async () => undefined,
      startRegularQueueGroup: async () => undefined,
      pauseRegularQueueGroup: async () => undefined,
    ...
    assert.equal(feature.refreshRegularQueueGroups, undefined);
    assert.equal(
    ...
    assert.equal(
      remainingPreviewLabel({ remaining: new Array(50), remainingCount: 386 }),
      "当前展示前 50 条，共 386 条待执行",
    );
    assert.equal(
      remainingPreviewLabel({ remaining: [{ position: 1 }], remainingCount: 1 }),
      "待执行 · 1",
    );
    feature.dispose();
  });
  ...
  it("does not let an older queue-group query overwrite a completed pause", async () => {
    ...
    center.setScope({ workspaceRuntimeId: "platform-stale-group-query-b" });
    assert.deepEqual(center.getSnapshot().data.regular.groups, []);
    center.dispose();
    feature.dispose();
  });
