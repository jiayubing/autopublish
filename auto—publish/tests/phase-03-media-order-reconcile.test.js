...
test("media order reconciliation commits verified published evidence and rejects weak URLs", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "phase-03-order-reconcile-"));
  ...
  try {
    ...
    assert.throws(() => store.reconcileRemoteOrder({ orderId: "order-1", outcome: { status: "published", remoteUrl: "http://weak.test" } }), { code: "OPERATIONAL_ORDER_EVIDENCE_REQUIRED" });
    assert.equal(store.reconcileRemoteOrder({ orderId: "order-1", outcome: { status: "published", remoteUrl: "https://proof.test/order-1" } }).status, "published");
    assert.equal(store.listRemoteOrders()[0].status, "published");
    assert.equal(store.listRemoteOrders()[0].remoteUrl, "https://proof.test/order-1");
  } finally { store.close(); }
});
