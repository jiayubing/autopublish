...
const {
  createAuthService,
  AUTH_BASE_URL,
} = require("../desktop/services/auth-service");
...
function authenticatedResponse(refreshToken, expiresAt) {
  return {
    ...
    body: {
      ...
      user: { id: refreshToken, loginName: refreshToken },
      entitlements: [],
    },
  };
}
...
describe("J4125 auth service client", function () {
  ...
  it("fails closed when refresh-token persistence cannot be committed", async function () {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), "autopublish-auth-persistence-failure-"),
    );
    const service = createAuthService({
      userDataPath: root,
      safeStorage: {
      ...
      request: async () =>
        authenticatedResponse(
          "refresh-persist-failure",
          new Date(Date.now() + 3600000).toISOString(),
        ),
    });
    try {
      await assert.rejects(() => service.login("admin", "password"), {
  ...
  it("maps server failures to fixed non-sensitive error codes", async function () {
    const service = createAuthService({
      request: async () => ({
        statusCode: 401,
        body: { error: "password hash details C:\\private\\db" },
      }),
    });
  ...
  it("preserves stable lock and rate-limit codes regardless of HTTP status", async function () {
    const locked = createAuthService({
      request: async () => ({
        statusCode: 423,
        body: {
          error: { code: "AUTH_ACCOUNT_LOCKED", message: "internal detail" },
        },
      }),
    });
    await assert.rejects(
      () => locked.login("admin", "password"),
      (error) => error.code === "AUTH_ACCOUNT_LOCKED",
    );
    const limited = createAuthService({
      request: async () => ({
        statusCode: 429,
        body: { error: { code: "AUTH_RATE_LIMITED" } },
      }),
    });
    await assert.rejects(
      () => limited.login("admin", "password"),
  ...
  it("coalesces concurrent protected calls into one refresh request", async function () {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), "autopublish-auth-concurrency-"),
    );
      ...
      request: async (input) => {
        if (input.url.endsWith("/login"))
          return authenticatedResponse(
            "refresh-1",
            new Date(currentTime + 3600000).toISOString(),
          );
        refreshCalls += 1;
  ...
  it("keeps the encrypted refresh token and account state through temporary failures", async function () {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), "autopublish-auth-recovery-"),
    );
    let currentTime = Date.now();
    ...
    try {
      ...
      await assert.rejects(
        () => service.requireAuthenticated(),
        (error) => error.code === "AUTH_SERVICE_UNAVAILABLE",
      );
  ...
  it("clears the session only for a terminal refresh error and ignores stale responses", async function () {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), "autopublish-auth-generation-"),
    );
    let currentTime = Date.now();
    ...
    const service = createAuthService({
      ...
      request: async (input) => {
        ...
        return {
          statusCode: 401,
          body: { error: { code: "AUTH_SESSION_EXPIRED" } },
        };
      },
    });
    try {
    ...
    const service = createAuthService({
      ...
      request: async (input) =>
        input.url.endsWith("/login")
          ? authenticatedResponse(
              "refresh-1",
              new Date(currentTime + 120000).toISOString(),
            )
          : (() => {
