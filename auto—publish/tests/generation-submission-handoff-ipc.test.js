...
describe("generation submission handoff IPC", function() {
  it("rejects renderer paths and unknown fields before invoking the service", async function() {
    ...
    const pathResult = await handlers.get("content:preview-generation-submission-handoff")(null, { generationBatchId: "C:\\private\\generation", targetPlatformIds: ["target-a"] });
    const fieldResult = await handlers.get("content:preview-generation-submission-handoff")(null, { generationBatchId: "generation-1", targetPlatformIds: ["target-a"], articleId: "article-1" });
