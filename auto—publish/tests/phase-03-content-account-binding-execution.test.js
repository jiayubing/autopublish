...
test("main rejects a renderer account profile that differs from the durable queue binding", () => {
  ...
  try {
    ...
    assert.throws(() => workbench.buildSelectedSubmissionsPlan([{ sourcePlatformId: "toutiao", filename: "article.md", targetPlatformIds: ["toutiao"], accountProfiles: { toutiao: "account-renderer" } }]), { code: "ACCOUNT_PROFILE_MISMATCH" });
  } finally {
