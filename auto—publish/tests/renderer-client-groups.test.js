...
test("collection selection stays collapsed until requested, spans filters and sends explicit locked customer IDs", { concurrency: false }, async (t) => {
  ...
  assert.equal(await page.getByRole("combobox", { name: "批次客户分组" }).count(), 0);
  ...
  await picker.getByRole("combobox", { name: "批次客户分组" }).selectOption(high);
  await picker.getByRole("button", { name: "全选当前结果（55）" }).click();
  assert.match(await picker.innerText(), /已选 55 个客户/);
  await picker.getByRole("button", { name: "下一页" }).click();
  assert.equal(await picker.getByRole("checkbox", { name: "合成客户 55" }).isChecked(), true);
  ...
  assert.equal(await selector.getByRole("button", { name: "选择客户" }).isDisabled(), true);
...
test("group management lives inside batch picker and generation keeps independent picks", { concurrency: false }, async (t) => {
  ...
  assert.equal(await page.getByRole("combobox", { name: "客户分组", exact: true }).count(), 0);
  assert.equal(await page.getByRole("button", { name: "管理分组", exact: true }).count(), 0);
  assert.equal(await current.locator("optgroup").count(), 1);

  const collectionSelection = page.locator("[data-client-selection]");
  await collectionSelection.getByRole("button", { name: "选择客户" }).click();
  const picker = page.getByRole("dialog", { name: "选择批次客户" });
  await picker.getByRole("button", { name: "管理分组", exact: true }).click();
  ...
  await picker.getByRole("combobox", { name: "批次客户分组" }).selectOption(high);
  await picker.getByRole("button", { name: "全选当前结果（2）" }).click();
  await picker.getByRole("button", { name: "完成选择" }).click();
  assert.match(await collectionSelection.innerText(), /已选 2 个客户/);

  await page.getByRole("button", { name: "批量生成", exact: true }).click();
  const generationSelection = page.locator("[data-client-selection]");
  assert.match(await generationSelection.innerText(), /已选 1 个客户/);
  await generationSelection.getByRole("button", { name: "清空选择", exact: true }).click();
  await generationSelection.getByRole("button", { name: "选择客户" }).click();
  const generationPicker = page.getByRole("dialog", { name: "选择批次客户" });
  await generationPicker.getByRole("combobox", { name: "批次客户分组" }).selectOption("ungrouped");
  await generationPicker.getByRole("button", { name: "全选当前结果（1）" }).click();
  await generationPicker.getByRole("button", { name: "完成选择" }).click();
...
test("group read failure keeps batch picker searchable and retryable", { concurrency: false }, async (t) => {
  ...
  await picker.getByRole("button", { name: "查看已选" }).click();
  assert.match(await picker.innerText(), /尚未选择客户/);
  await picker.getByRole("button", { name: "完成选择" }).click();
  assert.equal(await page.getByRole("button", { name: "采集选中客户", exact: true }).isDisabled(), true);
});

test("an empty content workspace stays compact while grouping remains reachable", { concurrency: false }, async (t) => {
  const { page } = await openFixture(t, 0);
  const selector = page.locator("[data-client-selection]");
  assert.match(await selector.innerText(), /已选 0 个客户：尚未选择客户/);
  assert.equal(await selector.getByRole("checkbox").count(), 0);
  assert.equal(await page.getByRole("button", { name: "采集选中客户", exact: true }).isDisabled(), true);
  await selector.getByRole("button", { name: "选择客户" }).click();
  const picker = page.getByRole("dialog", { name: "选择批次客户" });
  ...
  assert.equal(await picker.getByRole("button", { name: "管理分组" }).isDisabled(), false);
});
