...
it("keeps Ticket 14 resolutions out of generic attention commands while preserving a reachable DTO projection", async () => {
  const query = createArticleAttentionQuery({
    operationalStore: {
      listPublicationAttention: () => [{
        ...
        resolutionActions: ["bind-paid-order-number", "confirm-paid-order-absent"],
      }],
    },
  });
  ...
  assert.deepEqual(item.resolutionActions, ["bind-paid-order-number", "confirm-paid-order-absent"]);
  ...
  assert.throws(
    () => resolver.preview({ attentionId: item.attentionId, action: "bind-paid-order-number" }),
