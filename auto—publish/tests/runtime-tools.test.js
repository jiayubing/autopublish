...
const {
  prepareRuntimeTools,
  readBuildInfo,
} = require("../scripts/prepare-runtime-tools");
...
it("records build provenance from the actual repository state", function () {
  const buildInfo = readBuildInfo();
  assert.match(buildInfo.commit, /^[a-f0-9]{40,64}$/);
...
it("prepares only regular node.exe and LICENSE files from a verified archive", async function () {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "autopublish-runtime-tools-"),
  );
  ...
  try {
    const result = await prepareRuntimeTools({
      ...
      runner: function (_command, _args, options) {
        const extracted = path.join(
          options.env.AUTOPUBLISH_RUNTIME_EXTRACT,
    ...
    assert.equal(
      fs.readFileSync(path.join(output, "LICENSE"), "utf8"),
      "Node.js license\n",
    );
    assert.equal(
      JSON.parse(
        fs.readFileSync(
          path.join(output, "runtime-tools-manifest.json"),
          "utf8",
        ),
      ).nodeVersion,
      "v24.18.0",
...
it("rejects a downloaded archive whose checksum differs from the manifest", async function () {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "autopublish-runtime-tools-checksum-"),
  );
  const manifestPath = path.join(root, "manifest.json");
  ...
  try {
    await assert.rejects(
      prepareRuntimeTools({
        ...
        download: async function (_url, filename) {
          fs.writeFileSync(filename, "tampered archive");
        },
      }),
      function (error) {
        return error.code === "RUNTIME_TOOL_CHECKSUM_MISMATCH";
      },
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
