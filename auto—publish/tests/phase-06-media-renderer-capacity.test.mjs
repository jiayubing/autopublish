...
} from "../media-workbench/src/features/media/media-feature.js";
  ...
  for (const total of [1_000, 10_000, 13_000, 20_000]) {
    it(`keeps the Renderer snapshot bounded to one page at ${total.toLocaleString()} resources`, async function (t) {
      ...
      const feature = createMediaFeature({
        ...
        searchResourcePage: async ({ page, pageSize }) =>
          syntheticPage(total, page, pageSize),
        refreshResources: async () => ({ status: "complete" }),
        getPoolPage: async ({ page, pageSize }) => ({
          items: [],
          memberResourceIds: [],
          total: 0,
          page,
      ...
      assert.ok(
        payloadBytes < 20_000,
        `single-page payload must stay bounded, got ${payloadBytes}`,
      );
      t.diagnostic(
        JSON.stringify({
