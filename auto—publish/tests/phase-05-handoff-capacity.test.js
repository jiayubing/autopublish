...
it("runs 500 and 5000 tasks through the production file adapter with one identity scan per preview", function() {
  ...
  try {
    ...
    const created = []; const regularQueueApplication = { previewRegularQueueAdmission: ({ articleRefs }) => ({ queueableCount: articleRefs.length, idempotentCount: 0, missingCount: 0, conflictCount: 0, items: articleRefs.map((articleRef) => ({ articleRef, articleId: articleRef.articleId, status: "queueable" })) }), admitRegularQueueItems: ({ articleRefs }) => { created.push({ clientId: articleRefs[0].clientId, count: articleRefs.length }); return { admittedCount: articleRefs.length, idempotentCount: 0, items: [] }; } };
    let taskCount = 500;
    const service = createGenerationSubmissionHandoffService({ generationBatchService: { get: () => ({ id: "batch", revision: 1, status: "completed", tasks: Array.from({ length: taskCount }, (_, index) => ({ id: "task-" + index, clientId: "client-1", status: "succeeded" })) }) }, contentStore, regularQueueApplication, targetPlatforms: [{ id: "target", contentQueueImport: true }] });
