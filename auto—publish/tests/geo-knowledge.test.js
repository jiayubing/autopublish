...
test("Responses transport separates trusted citations and never retries uncertain requests", async () => {
  let calls = 0;
  const client = createDoubaoGeoClient({ getConfig: () => ({ apiKey: "synthetic", model: "synthetic-model" }), fetch: async (url, options) => {
...
test("configuration stores encrypted credentials and status never returns the key", t => {
  ...
  config.save({ model: "synthetic", apiKey: "secret-fixture", webSearch: true });
  ...
  config.save({ model: "changed", apiKey: "", webSearch: false });
