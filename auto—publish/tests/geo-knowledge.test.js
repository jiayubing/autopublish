...
function document(name) {
  return normalizeCandidate(candidate(name), [source], "client-1");
}
...
test("atomic persistence, revision and malformed content protect previous knowledge", (t) => {
  ...
  assert.throws(() => store.save({ ...saved, schemaVersion: 2 }, 1), {
...
test("small material input works; malformed output gets only one repair", async () => {
  let calls = 0;
  const research = createGeoKnowledgeResearch({
    client: {
      async request() {
        calls++;
        return {
  ...
  const result = await research.extract({
    clientId: "client-1",
    clientName: "合成门店",
    materials: [],
  });
  ...
  const broken = createGeoKnowledgeResearch({
    client: {
      async request() {
        throw Object.assign(new Error(), { code: "GEO_REQUEST_UNCERTAIN" });
      },
    },
  });
  await assert.rejects(
    broken.extract({ clientId: "client-1", materials: [] }),
...
test("Responses transport separates trusted citations and never retries uncertain requests", async () => {
  ...
  assert.equal(
    (await client.request({ prompt: "合成测试", search: true })).citations
      .length,
    1,
  );
...
test("configuration stores encrypted credentials and status never returns the key", (t) => {
  ...
  assert.equal(
    fs
      .readFileSync(path.join(root, "doubao-geo.json"), "utf8")
