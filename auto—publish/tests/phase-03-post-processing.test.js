...
function reserveAndPublish(store, profile, value) {
  ...
  store.commitRemoteOutcome({
    attemptId: value.attemptId,
    outcome: publishedOutcome(profile, value.attemptId, value.articleId),
...
test("archive post-processing waits for every target in its source group and is idempotent after a crash", async () => {
  ...
  try {
    ...
    await assert.rejects(() => processor.process(firstJob), {
      code: "POST_PROCESSING_ARCHIVE_NOT_ELIGIBLE",
    });
    ...
    reserveAndPublish(store, second, {
      articleId: "article-2",
    ...
    assert.deepEqual(await processor.process(retry), {
      archived: true,
      idempotent: true,
    });
  } finally {
...
test("failed post-processing is attention-visible and is not automatically re-claimed", () => {
  ...
  try {
    ...
    reserveAndPublish(store, profile, {
      articleId: "article-1",
      publicationId: "publication-1",
      attemptId: "attempt-1",
      batchItemId: batch.items[0].itemId,
