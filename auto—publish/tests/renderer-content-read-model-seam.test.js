...
test("production content views render the content read model without query bridge ownership", () => {
  ...
  const productionViews = [workbench, questions, generation, batch, history].join("\n");
  ...
  assert.doesNotMatch(workbench, /articleRefreshToken|batchRefreshToken|useWorkspaceScope\('articleManagement'/);
  assert.doesNotMatch(productionViews, /\brefreshToken\b|\bbatchRefreshToken\b|onRefreshArticles|onRefreshBatchState/);
