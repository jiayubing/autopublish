    ...
    for (let j = i + 1; j < STORAGE_ROOTS.length; j += 1) {
      ...
      if (contains(first, second) || contains(second, first)) {
        throw storagePathError(
          "storage roots must be distinct and must not overlap",
        );
      }
    }
  }
...
function createStoragePaths(input) {
  const values = input || {};
  const roots = {
    installation: absoluteRoot(
      values.installation || values.appRoot,
      "installation",
    ),
    roamingConfig: absoluteRoot(
      values.roamingConfig || values.userDataPath,
      "roamingConfig",
    ),
    localState: absoluteRoot(
      values.localState || values.sessionDataPath,
      "localState",
    ),
    contentLibrary: absoluteRoot(
      values.contentLibrary || values.workspaceRoot,
      "contentLibrary",
    ),
  };
...
function validateStoragePaths(paths) {
  if (!paths || typeof paths !== "object")
    throw storagePathError("storage paths are required");
  const roots = {};
  STORAGE_ROOTS.forEach(function (name) {
    roots[name] = absoluteRoot(paths[name], name);
  });
  assertDistinctRoots(roots);
  STORAGE_ROOTS.forEach(function (name) {
    if (path.resolve(paths[name]) !== roots[name])
      throw storagePathError(name + " is not normalized");
  });
  return paths;
}
...
function mkdirSafe(directory) {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink())
    throw storagePathError("content library directory is invalid");
}

function ensureContentLibrary(paths) {
  const storage = validateStoragePaths(paths);
  [
    storage.contentLibrary,
    storage.clients,
    storage.generated,
    storage.templates,
    storage.autopublish,
