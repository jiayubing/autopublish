...
function hasIndex(db, table, expected, unique) {
  return db
    ...
    .all()
    .filter(
      (index) => unique === undefined || index.unique === (unique ? 1 : 0),
    )
    .some(
      (index) =>
        JSON.stringify(
...
function hasForeignKey(db, table, from, target, to) {
  return db
    ...
    .all()
    .some((key) => key.from === from && key.table === target && key.to === to);
}
...
function verifyV4Structure(db, errorCode) {
  const required = {
    ...
    submission_queue_groups: [
      ["queue_group_id", "TEXT", 1, 1],
      ["platform_id", "TEXT", 1, 0],
      ["account_profile_id", "TEXT", 1, 0],
      ["pause_intent", "TEXT", 1, 0],
    ...
    submission_queue_items: [
      ["item_id", "TEXT", 1, 1],
      ["queue_group_id", "TEXT", 1, 0],
      ["position", "INTEGER", 1, 0],
      ["created_at", "TEXT", 1, 0],
    ],
    paid_submission_batches: [
      ["batch_id", "TEXT", 1, 1],
      ["media_resource_id", "TEXT", 1, 0],
      ["confirmation_fingerprint", "TEXT", 1, 0],
      ["confirmation_json", "TEXT", 1, 0],
      ["system_submission_code", "TEXT", 1, 0],
      ["quoted_price", "REAL", 1, 0],
      ["estimated_total", "REAL", 1, 0],
      ["article_count", "INTEGER", 1, 0],
      ["pause_intent", "TEXT", 1, 0],
      ["created_at", "TEXT", 1, 0],
      ["confirmed_at", "TEXT", 1, 0],
      ["updated_at", "TEXT", 1, 0],
    ],
    manual_reconciliation_facts: [
      ["reconciliation_id", "TEXT", 1, 1],
      ["attempt_id", "TEXT", 1, 0],
      ["article_id", "TEXT", 1, 0],
      ["decision", "TEXT", 1, 0],
    ...
    return (
      ...
      expected.every(
        ([column, type, notnull, pk], index) =>
          actual[index] &&
          actual[index].name === column &&
          actual[index].type === type &&
          actual[index].notnull === notnull &&
          actual[index].pk === pk,
      )
    );
  });
  ...
  const orderExpected = [
    ...
    ["created_at", "TEXT", 1, 0],
    ["media_resource_id", "TEXT", 0, 0],
    ["estimated_total", "REAL", 0, 0],
    ["system_submission_code", "TEXT", 0, 0],
  ];
  const validOrder =
    order.length === orderExpected.length &&
    orderExpected.every(
      ([column, type, notnull, pk], index) =>
        order[index] &&
        order[index].name === column &&
        order[index].type === type &&
        order[index].notnull === notnull &&
        order[index].pk === pk,
    );
  const validForeignKeys =
    hasForeignKey(
      db,
      "article_active_targets",
      "publication_id",
    ...
    hasForeignKey(
      db,
      "order_display_snapshots",
      "attempt_id",
      "publication_attempts",
      "attempt_id",
    );
  const validIndexes =
    hasIndex(db, "article_active_targets", ["article_id"], true) &&
    hasIndex(db, "article_active_targets", ["publication_id"], true) &&
    hasIndex(db, "article_active_targets", ["attempt_id"], true) &&
    hasIndex(
      db,
      "submission_queue_groups",
      ["platform_id", "account_profile_id"],
      true,
    ) &&
    hasIndex(
      db,
      "submission_queue_items",
      ["queue_group_id", "position"],
      true,
    ) &&
    hasIndex(db, "manual_reconciliation_facts", ["attempt_id"], true);
  const validChecks =
    /CHECK\s*\(\s*state\s+IN\s*\(\s*'queued'\s*,\s*'remote_started'\s*,\s*'submitting'\s*,\s*'submitted'\s*,\s*'uncertain'\s*\)\s*\)/i.test(
      sqlOf(db, "article_active_targets"),
    ) &&
    /CHECK\s*\(\s*pause_intent\s+IN\s*\(\s*'none'\s*,\s*'manual'\s*,\s*'system'\s*\)\s*\)/i.test(
      sqlOf(db, "submission_queue_groups"),
    ) &&
    /CHECK\s*\(\s*decision\s+IN\s*\(\s*'accepted'\s*,\s*'not_accepted'\s*,\s*'order_bound'\s*,\s*'no_order'\s*\)\s*\)/i.test(
      sqlOf(db, "manual_reconciliation_facts"),
    );
  if (
    !validTables ||
    !validOrder ||
...
function installV4Schema(db, errorCode) {
  ...
  const existing = new Set(
    columns(db, "order_display_snapshots").map((column) => column.name),
  );
  for (const [name, type] of [
    ["media_resource_id", "TEXT"],
    ["estimated_total", "REAL"],
    ["system_submission_code", "TEXT"],
  ])
    if (!existing.has(name))
      db.exec(`ALTER TABLE order_display_snapshots ADD COLUMN ${name} ${type}`);
  ...
  return crypto
    ...
    .update(
      text(
        db
          .prepare(
            "SELECT attempt_id,title_snapshot,filename,resource_name_snapshot,quoted_price,created_at FROM order_display_snapshots ORDER BY attempt_id",
          )
          .all(),
      ),
    )
...
function migrateV4Schema(db, migrationHook, helpers) {
  const { tableDataHashes, verifyTableDataHashes, verifyMigrationHistory } =
    helpers;
  const fault = (point) => {
  ...
  helpers.runTransaction(db, () => {
    ...
    if (orderDisplayDataHash(db) !== beforeOrderDisplay)
    ...
    if (orderDisplayDataHash(db) !== beforeOrderDisplay)
      throw fail("OPERATIONAL_SCHEMA_MIGRATION_INVALID");
    db.prepare("INSERT INTO schema_migrations VALUES(4,?)").run(
