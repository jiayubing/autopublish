...
const DECISIONS = new Set(["accepted", "not_accepted", "order_bound", "no_order"]);
...
function createOperationalStoreReconciliationAggregate(context) {
  ...
  function recordManualReconciliation(input) {
    ...
    if (!DECISIONS.has(value.decision)) throw fail("OPERATIONAL_RECONCILIATION_DECISION_INVALID");
    ...
    if (text(evidence).length > 32768) throw fail("OPERATIONAL_RECONCILIATION_EVIDENCE_INVALID");
    const reconciliationId = value.reconciliationId === undefined
      ? `reconcile-${randomUUID()}`
      : requiredText(value.reconciliationId, 128, "OPERATIONAL_RECONCILIATION_ID_INVALID");
    ...
    return transaction(() => {
      ...
      if (value.articleId !== undefined && value.articleId !== attempt.article_id)
        throw fail("OPERATIONAL_RECONCILIATION_ARTICLE_MISMATCH");
      if (value.targetKey !== undefined && value.targetKey !== attempt.target_key)
        throw fail("OPERATIONAL_RECONCILIATION_TARGET_MISMATCH");
      const existing = db.prepare("SELECT * FROM manual_reconciliation_facts WHERE attempt_id=?").get(attemptId);
      if (existing) {
        if (existing.decision !== value.decision || existing.reconciliation_id !== reconciliationId)
      ...
      try {
        ...
        ).run(reconciliationId, attemptId, attempt.article_id, value.decision, text(evidence), stamp);
      } catch (error) {
        if (String(error && error.code || "").startsWith("SQLITE_CONSTRAINT"))
          throw fail("OPERATIONAL_RECONCILIATION_CONFLICT");
        throw error;
      }
      return rowView(db.prepare("SELECT * FROM manual_reconciliation_facts WHERE reconciliation_id=?").get(reconciliationId), false);
    });
  }
  ...
  function listManualReconciliations(input) {
    ...
    if (Array.isArray(value.articleIds) && value.articleIds.length) {
      const articleIds = value.articleIds.map((id) => domain.ArticleId.serialize(domain.ArticleId.parse(id)));
    ...
    if (Array.isArray(value.attemptIds) && value.attemptIds.length) {
      const attemptIds = value.attemptIds.map((id) => domain.AttemptId.serialize(domain.AttemptId.parse(id)));
    ...
    return Object.freeze(
      db.prepare(
        "SELECT * FROM manual_reconciliation_facts " + where + " ORDER BY created_at,reconciliation_id LIMIT 20000",
      ).all(...params).map(rowView),
    );
  }

  return Object.freeze({ recordManualReconciliation, listManualReconciliations });
}
