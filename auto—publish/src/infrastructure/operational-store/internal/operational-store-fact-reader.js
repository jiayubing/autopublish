...
function createOperationalStoreFactReader(context) {
  ...
  function targetFields(target) {
    ...
    return {
      ...
      mediaResourceId: value.kind === "media" ? value.mediaResourceId || null : null,
    };
  }
  ...
  function listArticleLifecycleFacts(input) {
    ...
    if (!articleIds.length)
      return Object.freeze({ publications: Object.freeze([]), submissionItems: Object.freeze([]), orders: Object.freeze([]), attentionItems: Object.freeze([]), manualReconciliations: Object.freeze([]) });
    ...
    const submissionItems = db.prepare(
    ...
    ).all(...articleIds).map((row) => Object.freeze({
      itemId: row.item_id,
    ...
    const orders = db.prepare(
      ...
      return Object.freeze({
        ...
        publicationId: row.publication_id,
        articleId: row.article_id,
        mediaResourceId: row.media_resource_id || target.mediaResourceId || null,
        targetKey: target.kind === "media" ? `media-resource:${target.mediaResourceId}` : null,
        ...
        resourceNameSnapshot: safeDisplayText(row.resource_name_snapshot, 500),
        quotedPrice: row.quoted_price === null ? null : row.quoted_price,
        estimatedTotal: row.estimated_total === null ? null : row.estimated_total,
    ...
    const attentionItems = db.prepare(
      ...
      return Object.freeze({
        ...
        targetKey: row.target_key,
        status: row.status,
        reasonCode: typeof detail.code === "string" ? detail.code : "PUBLICATION_UNCERTAIN",
        updatedAt: row.updated_at,
      });
    });
    const manualReconciliations = db.prepare(
    ...
    if (internalLifecycleProjectionObserver)
      internalLifecycleProjectionObserver({ sqlCount: 5, rowCount: publications.length + submissionItems.length + orders.length + attentionItems.length + manualReconciliations.length, parsedPayloadCount: submissionItems.length + orders.length + attentionItems.length + manualReconciliations.length });
    return Object.freeze({
