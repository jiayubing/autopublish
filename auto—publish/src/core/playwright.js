...
} = require("../infrastructure/runtime/playwright-runtime-resolver");
...
// and pass the returned context to pwInvokeSync/runCode via opts.session.
...
function pwSessionConfig(name, options) {
  var input =
    name && typeof name === "object"
      ? name
      : Object.assign({}, options || {}, { session: name });
  var session = input.session || PW.session;
  var profileId =
    input.profileId === undefined ? "default" : String(input.profileId);
  if (
  ...
  var profileDir =
    session === PW.session && PW.profileDir
      ? PW.profileDir
      : path.join(PW.home, "profiles", session);
  var daemonDir =
    session === PW.session && PW.daemonDir
      ? PW.daemonDir
      : path.join(PW.home, "sessions", session);
...
function normalizeCommandArgs(commandArgs) {
  if (!Array.isArray(commandArgs)) {
    var error = new TypeError("Playwright command arguments must be an array");
    error.code = "PLAYWRIGHT_ARGUMENTS_INVALID";
    throw error;
  }
  return commandArgs.map(function (value) {
    if (typeof value !== "string") {
      var error = new TypeError("Playwright command arguments must be strings");
      error.code = "PLAYWRIGHT_ARGUMENTS_INVALID";
      throw error;
    }
    return value;
...
function pwInvokeSync(commandArgs, opts) {
  var options = opts || {};
  ...
  try {
    ...
    return (options.execFileSync || execFileSync)(
      ...
      executable.prefix.concat(
        runtimeArgs(sessionCtx, normalizeCommandArgs(commandArgs)),
      ),
      {
        encoding: "utf8",
        timeout: timeout,
    ...
    if (error && error.code === "PLAYWRIGHT_ARGUMENTS_INVALID") throw error;
...
function runCode(jsCode, opts) {
  var options = opts || {};
  var sessionCtx = options.session || null;
  var tempDir = options.tempDir || DIRS.tmpDir;
  var filePath = path.join(
    tempDir,
    "run-" + Date.now() + "-" + Math.random().toString(16).slice(2) + ".js",
  );
  ...
  try {
    return extractResult(
      pwInvokeSync(["run-code", "--filename=" + filePath], {
...
function mapRuntimeError(error) {
  var source =
    error instanceof Error
      ? error
      : new Error(String(error || "Playwright command failed"));
  if (
    source.code === "PLAYWRIGHT_NODE_UNAVAILABLE" ||
    source.code === "PLAYWRIGHT_CLI_UNAVAILABLE" ||
    source.code === "BROWSER_CHANNEL_UNAVAILABLE"
  ) {
  ...
  var timedOut =
    ...
    (source.killed &&
      (source.signal === "SIGTERM" || source.signal === "SIGKILL"));
  ...
  var mapped = new Error(
    timedOut
      ...
      : sessionNotOpen
        ? "Playwright session is not open"
        : "Playwright command failed",
  );
  mapped.code = timedOut
    ? "PLAYWRIGHT_TIMEOUT"
    : sessionNotOpen
...
function windowsNpmCliEntrypoint(cli) {
  ...
  if (
    !path.isAbsolute(cli) &&
    !path.win32.isAbsolute(cli) &&
    !cli.includes("\\") &&
    !cli.includes("/")
  ) {
    try {
      var located = execFileSync("where.exe", [cli], {
        encoding: "utf8",
