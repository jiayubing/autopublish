...
function createHepanGeoApiClient(options) {
  ...
  async function request(action, payload) {
    ...
    timeoutId = setTimeout(() => {
      controller.abort();
      rejectTimeout(fail("HEPAN_GEO_API_TIMEOUT"));
    }, timeoutMs);
    ...
    try {
      ...
      if (error && error.code === "HEPAN_GEO_API_TIMEOUT") throw error;
      throw fail(
        controller.signal.aborted || (error && error.name === "AbortError")
          ? "HEPAN_GEO_API_TIMEOUT"
          : "HEPAN_GEO_API_UNAVAILABLE",
      );
    }
    if (!response || response.ok !== true) {
      clearTimeout(timeoutId);
      throw fail("HEPAN_GEO_API_UNAVAILABLE");
    }

    let result;
    try {
      result = await Promise.race([response.json(), timeout]);
    } catch (error) {
      clearTimeout(timeoutId);
      if (
        ...
        throw fail("HEPAN_GEO_API_TIMEOUT");
      throw fail("HEPAN_GEO_API_PROTOCOL_ERROR");
    } finally {
      clearTimeout(timeoutId);
    }
    ...
    if (result.success !== true || result.code !== 0) {
      ...
      throw fail(
        BUSINESS_ERRORS[businessCode] || "HEPAN_GEO_API_PROTOCOL_ERROR",
        requestId,
      );
    }
    if (
    ...
    return Object.freeze({
      data: Object.freeze(Object.assign({}, result.data)),
  ...
  return Object.freeze({
    async status(config) {
      return request("status", credentials(config));
    },
