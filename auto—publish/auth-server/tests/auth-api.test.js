...
describe("isolated auth API", () => {
  ...
  before(() => {
    ...
    baseUrl = new Promise((resolve) => server.once("listening", () => resolve(`http://127.0.0.1:${server.address().port}`)));
  });
  after(() => { if (server) server.close(); if (root) fs.rmSync(root, { recursive: true, force: true }); });

  async function request(method, route, body, token) {
    const base = await baseUrl;
    return new Promise((resolve, reject) => {
      const request = http.request(`${base}${route}`, { method, headers: Object.assign({ "content-type": "application/json" }, token ? { authorization: `Bearer ${token}` } : {}) }, (response) => {
        let text = "";
