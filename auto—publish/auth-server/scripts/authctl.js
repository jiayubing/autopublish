...
async function run(argv, io) {
  ...
  const repository =
    (io && io.repository) ||
    new SqliteAuthRepository({ filePath: process.env.AUTH_DB_PATH });
  const domain = (io && io.domain) || new AuthDomain({ repository });
  const administration =
    (io && io.administration) || new AuthAdministration({ repository, domain });
  let result;
  try {
    if (
      (area === "user" || area === "admin") &&
      (action === "create" || action === "create-user")
    ) {
      const password = await confirmedSecret("密码: ", io || {});
      result = await administration.execute({
        type: area === "admin" ? "create-admin" : "create-user",
        loginName:
          options.loginName || options.login || process.env.AUTH_ADMIN_LOGIN,
        password,
    ...
    if (!(io && io.quiet))
      ((io && io.output) || process.stdout).write(
...
if (require.main === module) {
  run(process.argv.slice(2), {}).catch((error) => {
    process.stderr.write(
      `${error.code || "AUTH_ADMIN_FAILED"}: admin command failed\n`,
    );
    process.exitCode = 1;
  });
}
