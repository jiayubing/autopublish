...
function translate(argv) {
  ...
  switch (command) {
    case "create": {
      ...
      return ["user", "create", "--login-name", loginName].concat(
        args.slice(1),
      );
    }
    case "list":
      return ["user", "list"].concat(args);
    case "show":
      return [
        "user",
        "show",
        "--login-name",
        requireName(args, 0, "login name"),
      ].concat(args.slice(1));
    case "enable":
      return [
        "user",
    ...
    case "renew": {
      ...
      if (expiry === "permanent")
        return ["user", "set-expiry", "--login-name", loginName, "--permanent"];
      return [
        "user",
        "set-expiry",
        "--login-name",
    ...
    case "audit":
      return ["audit", "list"].concat(args);
    default:
      throw commandError(`unsupported command: ${command}`);
  }
}
