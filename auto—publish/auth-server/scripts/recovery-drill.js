...
function parseArgs(argv) {
  const args = argv || [];
  if (args.length !== 2 || args[0] !== "--temp-root" || !args[1])
    throw Object.assign(new Error("AUTH_RECOVERY_TEMP_ROOT_REQUIRED"), {
      code: "AUTH_RECOVERY_TEMP_ROOT_REQUIRED",
    });
  return args[1];
}
  ...
  main(process.argv.slice(2)).catch((error) => {
    const code =
      ...
      /^AUTH_[A-Z0-9_]{1,72}$/.test(error.code)
        ? error.code
        : "AUTH_RECOVERY_DRILL_FAILED";
    process.stderr.write(`${code}: recovery drill failed\n`);
    process.exitCode = 1;
  });
}
