...
} = require("../src/content/content-file-transaction");
...
function createDoubaoGeoConfigStore({ userDataPath, safeStorage }) {
  if (!path.isAbsolute(userDataPath))
    throw geoError("GEO_CONFIG_STORAGE_INVALID");
  ...
  function checkPath(target, directory = false) {
    try {
      ...
      if (error.code !== "ENOENT") throw geoError("GEO_CONFIG_STORAGE_INVALID");
    }
  }
  function encryption() {
    if (!safeStorage?.isEncryptionAvailable())
      throw geoError("GEO_ENCRYPTION_UNAVAILABLE");
  }
  function read() {
    ...
    try {
      ...
      if (
        ...
        !stored.model.trim() ||
        typeof stored.webSearch !== "boolean"
      )
        throw geoError("GEO_CONFIG_INVALID");
      const baseUrl = normalizeGeoBaseUrl(
        stored.version === 1
          ? STANDARD_BASE_URL
          : stored.baseUrl === LEGACY_CODING_BASE_URL
            ? CODING_BASE_URL
            : stored.baseUrl,
      );
  ...
  function status() {
    ...
    return {
      ...
      baseUrl: value?.baseUrl || CODING_BASE_URL,
    };
  }
  function save(input) {
    encryption();
    ...
    try {
      ...
      throw geoError("GEO_CONFIG_SAVE_FAILED");
    }
    return status();
  }
