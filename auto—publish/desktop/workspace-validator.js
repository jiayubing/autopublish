...
const { readWorkspaceSchemaMarker } = require("./workspace-schema-gate");
...
function validateMarker(io, markerPath) {
  const result = readWorkspaceSchemaMarker(markerPath, { fs: io });
  return result.allowed
    ? { ok: true, value: result.marker }
...
function createWorkspaceValidator(options) {
  ...
  const userDataValues =
    options.userDataPath === undefined || options.userDataPath === null
      ? []
  ...
  const systemPaths = collectProtectedPaths(
    options.systemPaths === undefined
      ? defaultSystemPaths()
      : options.systemPaths,
  );
  ...
  function isForbidden(candidatePath) {
    ...
    if (
      systemPaths.some(function (systemPath) {
        return isRelated(candidatePath, systemPath);
      })
    )
      return true;
    if (
      applicationPaths.some(function (applicationPath) {
        return isRelated(candidatePath, applicationPath);
      })
    )
      return true;
  ...
  function isWritable(candidatePath) {
    ...
    if (cleanupFailed)
      return { ok: false, code: "WORKSPACE_PROBE_CLEANUP_FAILED" };
  ...
  function validate(candidate) {
    ...
    if (marker.exists && !marker.ok) {
      ...
      const schemaMessage =
        schemaCode === "WORKSPACE_SCHEMA_FUTURE"
          ? "Workspace schema is newer than this application supports"
          : schemaCode === "WORKSPACE_SCHEMA_OLDER_UNSUPPORTED"
            ? "Workspace schema requires an explicit upgrade"
            : "Workspace marker is invalid";
      return invalid(schemaCode, schemaMessage, realPath);
    }
    if (marker.exists) {
      ...
      if (!writable.ok) {
        return invalid(
          writable.code,
          writable.code === "WORKSPACE_PROBE_CLEANUP_FAILED"
    ...
    if (entries.length === 0)
      return { kind: "empty_directory", path: realPath };
