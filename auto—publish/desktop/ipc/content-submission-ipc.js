...
function registerContentSubmissionIpc(deps) {
  const service = deps.contentSubmissionService || createContentSubmissionService({ workspaceRoot: deps.rootDir, platforms: deps.platforms });
  ...
  function safeBatchResult(value) {
    ...
    if (Array.isArray(result.items)) result.items.forEach(function(item) { delete item.filePath; delete item.sidecarPath; });
    return result;
  }
