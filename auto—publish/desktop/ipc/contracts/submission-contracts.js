...
const submissionContracts = Object.freeze([
  ...
  contract({ capability: "content.listSubmissionBatches", channel: "content:list-submission-batches", kind: "query", request: listBatchesRequest, success: exactObject({ batches: arrayField(batch, { max: 1000 }) }), fromArgs: (args) => args[0] || {}, toArgs: directInput }),
  ...
  contract({ capability: "content.previewCancelSubmissionBatch", channel: "content:preview-cancel-submission-batch", kind: "query", request: exactObject({ batchId: id }), success: cancelPreview, fromArgs: directArgs, toArgs: directInput }),
  ...
  contract({ capability: "content.previewRetryFailedPublication", channel: "content:preview-retry-failed-publication", kind: "query", request: exactObject({ publicationId: id }), success: retryPreview, fromArgs: directArgs, toArgs: directInput }),
  contract({ capability: "content.retryFailedPublication", channel: "content:retry-failed-publication", kind: "command", request: exactObject({ publicationId: id, expectedRevision: optionalField(revision), confirmed: literalField(true) }), success: retryResult, fromArgs: directArgs, toArgs: directInput }),
...
function projectSubmissionResult(channel, value) {
  ...
  if (channel === "content:list-submission-batches") return { batches: (Array.isArray(value) ? value : []).map(projectBatch) };
  ...
  if (channel === "content:preview-cancel-submission-batch") return { batchId: value.batchId, planId: value.planId, allowedCount: value.allowedCount, blockedCount: value.blockedCount, items: (value.items || []).map(projectBatchItem) };
  ...
  if (channel === "content:preview-retry-failed-publication") { const preview = { publicationId: value.publicationId == null ? null : value.publicationId, requiresConfirmation: value.requiresConfirmation === true }; for (const key of ["eligible", "reasonCode", "clientId", "articleId", "targetPlatformId", "titleSnapshot", "failureCount", "message"]) include(preview, value, key); if (value.preview) for (const key of ["queueableTaskCount", "idempotentCount", "conflictCount"]) include(preview, value.preview, key); return preview; }
  if (channel === "content:retry-failed-publication") return { batchId: value.batchId, publicationId: value.publicationId, attemptId: value.attemptId == null ? null : value.attemptId, clientId: value.clientId, articleId: value.articleId, targetPlatformId: value.targetPlatformId, changedScopes: value.changedScopes || [] };
...
const submissionContractFixtures = Object.freeze([
  ...
  { channel: "content:list-submission-batches", owner: "content", productionCaller: "desktopConsole.content.listSubmissionBatches", request: { clientId: "client-1" }, result: { batches: [{ id: "batch-1", clientId: "client-1", status: "queued", items: [{ articleId: "article-1", targetPlatformId: "toutiao", status: "queued" }] }] } },
  ...
  { channel: "content:preview-cancel-submission-batch", owner: "content", productionCaller: "desktopConsole.content.previewCancelSubmissionBatch", request: { batchId: "batch-1" }, result: { batchId: "batch-1", planId: "plan-1", allowedCount: 1, blockedCount: 0, items: [{ articleId: "article-1", targetPlatformId: "toutiao", status: "queued", allowed: true, fingerprint: "fingerprint-1" }] } },
  ...
  { channel: "content:preview-retry-failed-publication", owner: "content", productionCaller: "desktopConsole.content.previewRetryFailedPublication", request: { publicationId: "publication-1" }, result: { publicationId: "publication-1", requiresConfirmation: true, eligible: true, clientId: "client-1", articleId: "article-1", targetPlatformId: "toutiao", failureCount: 1 } },
  { channel: "content:retry-failed-publication", owner: "content", productionCaller: "desktopConsole.content.retryFailedPublication", request: { publicationId: "publication-1", expectedRevision: 3, confirmed: true }, result: { batchId: "batch-1", publicationId: "publication-1", attemptId: "attempt-2", clientId: "client-1", articleId: "article-1", targetPlatformId: "toutiao", changedScopes: ["articleManagement"] } },
