...
function createTypedIpcMain(ipcMain, requireAuthenticated) {
  ...
  function complete(context, result) {
    ...
    try {
      ...
      return productionIpcRegistry.failure(context.contract, {
  ...
  function fail(channelOrContext, error) {
    const context =
      channelOrContext && channelOrContext.contract
        ? channelOrContext
  ...
  const proxy = {
    ...
    handle(channel, handler) {
      ...
      const wrapped = async function (event, ...args) {
        ...
        try {
          if (args.length !== 1)
            throw Object.assign(new Error("Invalid IPC request"), {
...
function registerIpc(deps) {
  ...
  const authenticated = createTypedIpcMain(
    values.ipcMain,
    values.requireAuthenticated ||
      (values.authService && values.authService.requireAuthenticated),
  );
  ...
  const regularQueueApplication = values.regularQueueApplication
    ? require("../services/cross-client-regular-queue-application").createCrossClientRegularQueueApplication(
        { regularQueueApplication: values.regularQueueApplication },
      )
    : values.regularQueueApplication;
  ...
  try {
    modules.workspace = require("./workspace-runtime-ipc").registerWorkspaceRuntimeIpc(
      guarded,
    );
    ...
    if (values.operationalStore)
      modules.accountProfiles = require("./account-profile-ipc").registerAccountProfileIpc(
        guarded,
      );
    ...
    modules.platformSettings = require("./platform-settings-ipc").registerPlatformSettingsIpc(
      guarded,
    );
    modules.aiContent = require("./ai-content-ipc").registerAiContentIpc(guarded);
    modules.generation = require("./content-generation-batch-ipc").registerContentGenerationBatchIpc(
      Object.assign({}, guarded, {
        publishEvents: values.publishGenerationEvents !== false,
      }),
    );
    modules.submission = require("./content-submission-ipc").registerContentSubmissionIpc(
      guarded,
    );
    ...
    modules.attention = require("./article-attention-ipc").registerArticleAttentionIpc(
      Object.assign({}, guarded, {
    ...
    modules.articleManagement = require("./article-management-ipc").registerArticleManagementIpc(
      Object.assign({}, guarded, {
        articleAttentionQuery: modules.attention.query,
        publishedArchiveQueries: values.publishedArchiveQueries,
      }),
    );
    modules.doubao = require("./doubao-collection-ipc").registerDoubaoCollectionIpc(
      guarded,
    );
    modules.diagnostics = require("./runtime-diagnostics-ipc").registerRuntimeDiagnosticsIpc(
      guarded,
    );
