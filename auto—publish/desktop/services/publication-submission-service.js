  ...
  return Object.freeze({
    submit: async function (plan) {
      ...
      const preparedCommands = commands.map((command) => {
        ...
        // Validate every command before claiming any durable queue item. A
        // local DTO failure must not leave a partially started batch behind.
        parsePublishInput({
          version: 1,
      ...
      for (let index = 0; index < preparedCommands.length; index += 1) {
        const { command, attemptId } = preparedCommands[index];
        const batchId =
          command.postProcessingPayload &&
          command.postProcessingPayload.batchId;
        const item = value.operationalStore.findSubmissionItem({
          batchId,
          articleId: command.articleId,
          target: command.target,
        });
        const claimToken = `submission-${crypto.randomUUID()}`;
        const batchItem = value.operationalStore.claimSubmissionItemById({
        ...
        try {
          ...
          if (
            error &&
            (String(error.code || "").startsWith("ACCOUNT_PROFILE_") ||
