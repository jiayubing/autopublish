...
function applyPublicationBlocks(summary, articles, values) {
  if (
    ...
    typeof values.platformWorkbenchService.prepareMediaPublicationCommands !==
      "function"
  )
  ...
  return Promise.resolve(
    ...
    const articleIds = Array.from(
      new Set(
        commands.map((command) => command && command.articleId).filter(Boolean),
      ),
    );
    if (!articleIds.length) return summary;

    const records = values.operationalStore.listPublicationRecords({
      articleIds,
    });
...
function createMediaWorkbenchApplication(options) {
  ...
  async function resolveSubmissions(submissions) {
    ...
    return submissions.map(function (value) {
      ...
      const filePath = workbenchService.resolveSubmissionFile(
        submission.filename,
      );
      ...
      const scanned =
        articles.filter(
          (article) => article.filename === submission.filename,
        )[0] || {};
  ...
  return Object.freeze({
    ...
    buildConfirmation: async (submissions) => {
      ...
      return projectMediaPreflight(
        await applyPublicationBlocks(summary, articles, values),
      );
    },
    ...
    confirmPaidMedia: async (input) => {
      ...
      if (
        ...
        const execution = await paidMediaBatchOrchestrator.startBatch({
          batchId: result.batchId,
        });
        return Object.freeze(Object.assign({}, result, { execution }));
      }
      return result;
    },
    getPaidMediaBatches: () => {
    ...
    startPaidMediaBatch: (input) => {
      ...
      return paidMediaBatchOrchestrator.startBatch(input || {});
    },
    pausePaidMediaBatch: (input) => {
      ...
      return paidMediaBatchOrchestrator.pauseBatch(input || {});
    },
    submitSelected: async (submissions) => {
      const prepared = await resolveSubmissions(submissions);
      if (
        !values.mediaPublicationSubmissionService ||
        typeof values.mediaPublicationSubmissionService.submit !== "function"
      ) {
      ...
      const execution =
        await values.mediaPublicationSubmissionService.submit(prepared);
      const results = Array.isArray(execution && execution.results)
        ? execution.results
        : [];
      if (typeof values.invalidateData === "function")
        values.invalidateData("MEDIA_SUBMIT_COMPLETED");
      return {
        batchId: String((execution && execution.batchId) || ""),
        publishedCount: results.filter((item) =>
          ["published", "submitted"].includes(item.status),
        ).length,
        failedCount: results.filter((item) => item.status === "failed").length,
        uncertainCount: results.filter((item) => item.status === "uncertain")
          .length,
        skippedCount: 0,
      };
    },
