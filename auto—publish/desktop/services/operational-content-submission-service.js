...
} = require("./submission-batch-planner");
const {
...
function createOperationalContentSubmissionService(options) {
  ...
  const queuePaths = batchPersistence.queuePaths;
  ...
  function prepareMediaExport(input) {
    ...
    const platform = targetCatalog.find("media");
    ...
    const eligibility = preflight.check(article, {
      id: "media",
    ...
    const filename = batchPlanner.filenameFor(article);
  ...
  function failedPublicationRetryPlan(input) {
    ...
    if (!preflight.check(articleValue).eligible)
