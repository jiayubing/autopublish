...
function createOperationalContentSubmissionService(options) {
  ...
  function platforms() {
    if (Array.isArray(value.platforms)) return value.platforms.slice();
    return require("../../src/core/platforms").loadPlatforms().map((platform) => ({ id: platform.id, scanDir: platform.scanDir || platform.id, contentQueueImport: platform.contentQueueImport === true }));
  }
  ...
  return Object.freeze({ listPlatforms, previewBatch, createBatch, listBatches, getBatch,
