...
function absoluteDirectory(value, name) {
  if (
    ...
    throw maintenanceError(
      "STORAGE_MAINTENANCE_PATH_INVALID",
...
function stateName(value) {
  ...
  return String(
    value.status || value.state || value.phase || value.lifecycle || "",
  ).toLowerCase();
}
...
function isBusy(value, seen) {
  ...
  visited.add(value);
  if (
    BUSY_STATES.has(stateName(value)) ||
    value.isBatchRunning === true ||
    value.isPlatformRunning === true ||
    value.isStopPending === true ||
    value.isStopping === true ||
    value.running === true ||
    value.waiting === true ||
    value.paused === true
  )
  ...
  return Object.keys(value).some(function (key) {
    return isBusy(value[key], visited);
  });
}
...
function createStorageMaintenanceService(options) {
  ...
  const directories = {
    logs: absoluteDirectory(
      values.logs || path.join(localState, "logs"),
      "logs",
    ),
    temporary: absoluteDirectory(
      values.temporary || values.tmp || path.join(localState, "tmp"),
      "temporary",
    ),
    docxCache: absoluteDirectory(
      values.docxCache ||
        values.clientMaterialCache ||
        path.join(localState, "cache", "docx"),
      "docxCache",
    ),
    profiles: absoluteDirectory(
      values.browserProfile ||
        values.doubaoBrowser ||
        path.join(localState, "browser"),
      "profiles",
    ),
  };
  Object.keys(directories).forEach(function (key) {
    assertWhitelisted(localState, directories[key], key);
  });
  ...
  const activity =
    typeof opts.getActivityState === "function"
      ? opts.getActivityState
      : function () {
          return null;
        };
  ...
  function scan(directory) {
    ...
    return {
      files: files,
      followedSymlinks: followedSymlinks,
  ...
  function category(directory) {
    ...
    return {
      bytes: result.files.reduce(function (total, file) {
  ...
  function getUsage() {
    ...
    usage.totalBytes =
      ...
      usage.docxCache.bytes +
      usage.profiles.bytes;
    usage.removableBytes =
      usage.logs.bytes + usage.temporary.bytes + usage.docxCache.bytes;
    return usage;
  }
  ...
  function removeFiles(files, result) {
    files.forEach(function (file) {
      try {
        ...
        result.failed.push({
          path: file.path,
          code: error && error.code ? error.code : "STORAGE_DELETE_FAILED",
        });
      }
    });
  }
  ...
  function cleanupCaches() {
    if (isBusy(activity())) {
      return {
        blocked: true,
        reason: "STORAGE_MAINTENANCE_BUSY",
        deleted: [],
        failed: [],
        usage: getUsage(),
      };
    ...
    removeFiles(
      logs.filter(function (file) {
        return currentMs - file.mtimeMs > Number(limits.logAgeMs);
      }),
      result,
    );
    removeFiles(
      temporary.filter(function (file) {
        return currentMs - file.mtimeMs > Number(limits.temporaryAgeMs);
      }),
      result,
    );
    ...
    function enforceLimit(files, maxBytes) {
      ...
      files
        ...
        .sort(function (first, second) {
          return first.atimeMs - second.atimeMs;
        })
        .some(function (file) {
