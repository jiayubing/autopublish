...
function createWorkspaceRuntime(deps) {
    ...
    startPromise = (async function () {
      ...
      try {
        ...
        const contentLifecycleComposition = ownService(
          require("./composition/content-lifecycle-composition").createContentLifecycleComposition(
        ...
        ownDisposer(
          doubaoCollectionService.subscribe(function (value) {
            options.sendToRenderer(
              ...
              productionIpcRegistry.event(doubaoQueueContract,
  ...
  function registerIpc() {
    ...
    if (ipc && typeof ipc.dispose === "function")
      ownDisposer(function () {
        return ipc.dispose();
      });
    if (modules && modules.storageMaintenanceService) {
      const storageIpc =
        require("./ipc/storage-maintenance-ipc").registerStorageMaintenanceIpc({
          ipcMain: require("./ipc/register").createAuthenticatedIpcMain(
            options.ipcMain,
            options.authService && options.authService.requireAuthenticated,
          ),
          storageMaintenanceService: modules.storageMaintenanceService,
        });
      if (storageIpc && typeof storageIpc.dispose === "function")
        ownDisposer(function () {
          return storageIpc.dispose();
        });
    }
    return ipc;
  }
