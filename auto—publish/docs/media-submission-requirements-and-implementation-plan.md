...
### 修复 1：桌面端初始化异常 — 设置面板和媒体面板无响应

状态：已完成  
验收：已验收  
提交：39c2571

**问题**：app.js 第 705-712 行调用了三个不存在的函数 `updateMediaCacheInfo()`、`loadPoolIds()`、`renderMediaResources()`，导致 ReferenceError 中断脚本执行，使得后续的预检、订单中心、设置面板和 loadInitialState 全部未能初始化。

**表现**：
- 设置面板 API Key 始终显示"检测中..."
- "测试连接"和"查询余额"按钮点击无反应
- 订单中心可能不显示
- 其他 "Cannot read properties of undefined (reading 'then')" 异常

**修复**：删除 app.js 中残留的未定义函数调用块。媒体面板初始化已由 setTimeout 中的 mediaLoadCache() 和 mediaLoadPool() 负责。

### 修复 2：标题提取使用文件名而非文章内容

状态：已完成  
验收：已验收  
提交：39c2571

**问题**：`media:scan-articles` 处理器在从文件内容提取标题失败时，使用 `|| autoTitle` 回退到文件名，导致文件名被当作文章标题显示。

**修复**：改为始终优先从文章内容提取第一段作为标题，遍历非空行直到找到有效内容。仅当从内容完全无法提取任何文字时才使用文件名兜底。

### 修复 3：通用文件名扫描强制"城市_电话_联系人"格式

状态：已完成  
验收：已验收  
提交：39c2571

**问题**：`articles.js` 的 `scanArticles` 对所有平台强制解析"城市_电话_联系人"文件名格式，但该格式仅列举网使用，且当前使用的下划线分隔格式（`城市_电话_联系人`）已废弃。

**修复**：
- 从通用 `scanArticles` 移除 `parseFilenameMeta` 调用，直接返回文件列表（meta 字段为空字符串）
- 在列举网适配器中添加专用的 `scanArticles` 和 `parseArticleFiles`，使用紧凑格式（城市+电话+联系人，无分隔符）
- 其他平台（头条、合盘）使用各自的扫描逻辑，不受影响

### 修复 4：媒体拉取安全上限和部分数据显示
