...
const FORBIDDEN_NAMES = new Set([
  ".env", "runtime-config.json", "ai-provider.json", "workspace-location.json",
  ".autopublish-workspace.json", "cookie", "cookies"
]);
const FORBIDDEN_SEGMENTS = new Set([
  "input", "published", "data", "logs", "profile", "profiles", "state", "browser-profile",
  ".playwright-cli", "client-material-cache", "content-generation-batches"
]);
...
function staticEntries(appDir) {
  ...
  function visit(current) {
    fs.readdirSync(current, { withFileTypes: true }).forEach(function(entry) {
      ...
      const lowerParts = parts.map(function(value) { return value.toLowerCase(); });
      const lowerName = entry.name.toLowerCase();
      let stat;
      try { stat = fs.lstatSync(filename); } catch (_) { failures.push("UNREADABLE:" + rel); return; }
      if (stat.isSymbolicLink()) failures.push("SYMLINK:" + rel);
      if (FORBIDDEN_NAMES.has(lowerName) || lowerParts.some(function(value) { return FORBIDDEN_SEGMENTS.has(value); })) failures.push("PRIVATE:" + rel);
      ...
      if (absoluteTokens.some(function(token) { return token.test(content); })) failures.push("ABSOLUTE_REFERENCE:" + rel);
    });
  }
  visit(appDir);
  return failures;
...
function readPackagedManifest(appDir) {
  const filename = path.join(appDir, "tools", "node", "runtime-tools-manifest.json");
  if (!regularFile(filename)) throw verificationError("PACKAGED_NODE_MANIFEST_MISSING", "Packaged Node manifest is missing");
  let manifest;
  try { manifest = JSON.parse(fs.readFileSync(filename, "utf8")); } catch (_) { throw verificationError("PACKAGED_NODE_MANIFEST_INVALID", "Packaged Node manifest is invalid"); }
  if (!manifest || manifest.tool !== "node" || !/^v\d+\.\d+\.\d+$/.test(manifest.nodeVersion || "")) throw verificationError("PACKAGED_NODE_MANIFEST_INVALID", "Packaged Node manifest is invalid");
  return manifest;
}
...
function runCli(runtime, args, env, timeout) {
  try {
    ...
    throw verificationError("PACKAGED_PLAYWRIGHT_COMMAND_FAILED", "Packaged Playwright command failed");
  }
}

function verifyIsolatedRuntime(runtime, options) {
  const opts = options || {};
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "autopublish-packaged-playwright-"));
  const env = isolatedEnvironment(tempRoot);
  let opened = false;
  try {
    const version = execFileSync(runtime.node, ["--version"], { cwd: runtime.appDir, env: env, encoding: "utf8", timeout: 10000, windowsHide: true }).trim();
    if (version !== runtime.manifest.nodeVersion) throw verificationError("PACKAGED_NODE_VERSION_MISMATCH", "Packaged Node version does not match its manifest");
    ...
    if (opts.browserSmoke) {
      ...
      runCli(runtime, ["-s=packaged-verify", "open", "about:blank", "--browser=" + browser, "--headed", "--persistent", "--profile=" + profile], env, 60000);
      opened = true;
      runCli(runtime, ["-s=packaged-verify", "list"], env, 30000);
      runCli(runtime, ["-s=packaged-verify", "close"], env, 30000);
      opened = false;
    }
    return { ok: true, nodeVersion: version, browserSmoke: Boolean(opts.browserSmoke) };
  } finally {
    if (opened) {
      try { runCli(runtime, ["-s=packaged-verify", "close"], env, 10000); } catch (_) {}
    }
...
if (require.main === module) {
  ...
  const options = { browserSmoke: args.includes("--browser-smoke"), staticOnly: args.includes("--static-only") };
  try {
    const result = verifyPackagedRuntime(appDir, options);
    process.stdout.write(JSON.stringify({ ok: true, nodeVersion: result.static.manifest.nodeVersion, browserSmoke: Boolean(options.browserSmoke) }) + "\n");
  } catch (error) {
    process.stderr.write((error.code || "PACKAGED_RUNTIME_VERIFY_FAILED") + ":" + (error.message || "Packaged runtime verification failed") + "\n");
    process.exitCode = 1;
  }
}

module.exports = { verifyStaticPackage, verifyIsolatedRuntime, verifyPackagedRuntime, staticEntries };
