...
> 用途：记录文章生命周期重构的唯一实时执行进度，并定义用户说“执行波次 X”或“执行维护 Mxx”时的唯一调度行为。本计划负责业务 ticket 与维护插槽的调度，不自动审计、提交、合并或推送。业务语义仍以 `CONTEXT.md` 与 `ARTICLE-LIFECYCLE-AND-SUBMISSION-SPEC.md` 为真源。
...
主线程必须读取本计划、根目录 `AGENTS.md` 和该波次 ticket 或 maintenance 合同，验证调度依赖与 Git 状态，然后为当前 gate 允许的执行项创建独立 Codex worktree 线程。权威规格、owner、调用方和测试由执行线程在实施前勘察中按本任务范围读取；主线程不借波次/维护启动重新审计已完成阶段。维护任务只治理工程实现，不得改变业务规格、提前实现后续业务 ticket，也不得把“顺手重构”混入正在进行的波次审计修复。

“执行波次 X”或“执行维护 Mxx”只授权：

1. 检查集成分支、依赖、已有分支、worktree 和重复任务。
2. 创建该波次可执行 ticket 或当前维护插槽任务的独立 worktree 线程。
3. 在每个执行线程中创建对应分支、实施代码、运行定向测试并留下交接。

它不授权：

- ticket/maintenance 审计或审计子代理；Ticket 25 的专用 prompt 只执行最终验收用例、门禁和证据收集，完成后同样由用户另派审计 subagent；
- 自动修复审计发现；
...
上述操作均由用户后续对每个执行项单独下达指令；用户在当前请求中明确授权完整 maintenance 流程时，以该明确授权为准，但仍不得跨越合同中的串行 gate。

## 2. 当前进度快照

快照时间：2026-08-08。每次执行前必须以当前 Git 状态重新验证，快照不能覆盖 Git 事实。
...
| Maintenance 5.5 M01 实施         | `COMPLETE`                          | worktree `F:\官媒投稿-worktrees\article-lifecycle-maint-M01`；branch `codex/article-lifecycle-maint-M01`；base integration `3fe15a6185f759c30b1996374c301a3481f68100`；implementation commit `516bae7`；独立审计 task `/root/m01_audit` 发现 1 个 P2（同步 runCode 文件名碰撞），已修复并由同一审计复核 PASS；未访问真实账号、平台、供应商或订单                                                                                                                                                                                                                                                                                                                                                                                          |
| Maintenance 5.5 M01 合并与门禁   | `COMPLETE`                          | merge commit `f7cd39e`；最终集成 HEAD `f7cd39e`；2026-08-07T18:44:40.418+08:00 至 18:45:10.161+08:00，Node `v24.16.0`；定向组合 38/38，`npm run lint`、`npm run typecheck:main`、`npm run format:check`、`git diff --check` 全部通过；worktree 保留未跟踪整合包目录 `.scratch/incoming-maintenance/`，无 tracked/staged 变更                                                                                                                                                                                                                                                                                                                                                                                                              |
| Maintenance 5.5 M02 实施与审计    | `COMPLETE`                          | worktree `F:\官媒投稿-worktrees\article-lifecycle-maint-M02`；branch `codex/article-lifecycle-maint-M02`；implementation commit `2a499ef`；独立只读审计完成，无阻塞性 finding；未访问真实账号、平台、供应商或订单                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Maintenance 5.5 M02 合并与最终门禁 | `COMPLETE`                         | 原 merge commit `5a4fded` 与测试绑定 HEAD `0379240` 保留为历史 provenance。本次 closure commit `a409538`、测试基线 finding 修复 `b08f563`，integration merges `c84dd1a`、`fff0f96`；轻量审计无 P0–P2 finding。首次新 HEAD 完整门禁因 test discovery 仍锁定 251 files 而 1795/1796，修复后 clean `fff0f96103cf6e5303e6f2a1bf99354f2d5d709d`：`npm test` 收集 252 files、1796/1796 passed、0 failed/0 skipped/0 cancelled，wall clock 497902ms；随后官方 evidence runner 再次 1796/1796 PASS，Node `v24.16.0`，sourceState `CLEAN`（0 changed/staged/unstaged/untracked），2026-08-07T12:30:09.174Z 至 12:37:20.304Z，duration 431130ms；证据 `auto—publish/build/evidence/maintenance-55-final-root-test.json` |
| Wave 6 执行组 09                 | `COMPLETE`                          | implementation threadId `019fdc47-41b4-7ea2-b877-42650bcd19b1`；hostId `local`；worktree `C:\Users\violet\.codex\worktrees\deed\官媒投稿-refactor`；branch `codex/article-lifecycle-09`；base `e170d1ccdd4be11368a7df3acde240b90e5ca30f`；ticket commit `d60a237ca8fe34838bc8eb359f5dcf33a370e2c0`；integration merge `c29b6954133c60a27c3ec43cd6fe2b05ae6c3d85`                                                                                                                                                                                                                                                                                                                                 |
| Wave 6 Ticket 09 审计与修复      | `COMPLETE`                          | 首轮独立深审 task `/root/ticket09_audit` 报告 3 P1、2 P2；修复后复审 task `/root/ticket09_reaudit` 报告 1 P2；最终复审 task `/root/ticket09_final_audit` 无 P0–P2 finding。修复收口唯一 publication-success writer、迟到 accepted success-wins、accepted 安全远端身份、group-blocked 可恢复性、人工负面证据时间与 success evidence 精确读取；未访问真实账号、平台、供应商或订单                                                                                                                                                                                                                                                                                                                    |
| Wave 6 Ticket 09 合并后定向复验  | `COMPLETE`                          | 最终 integration HEAD `c29b6954133c60a27c3ec43cd6fe2b05ae6c3d85`；Ticket 09、Ticket 08、OperationalStore、composition、订单隔离、生命周期、workspace runtime 与三平台 adapter 定向组合 123/123；`npm run lint`、`npm run typecheck:main`、`npm run format:check`、`npm run test:discover`（256 files）与 `git diff --check` 全部通过。新增 `operational-store-regular-outcome-aggregate.js` 规模为 781 行，记录为 M03 深模块规模观察项，不在 Wave 6 擅自拆分                                                                                                                                                                                                                                                               |
| Wave 6 执行组 14                 | `COMPLETE`                          | implementation threadId `019fdc9f-0b3b-7f23-99e4-2a78b10dd738`；hostId `local`；worktree `C:\Users\violet\.codex\worktrees\361b\官媒投稿-refactor`；branch `codex/article-lifecycle-14`；base `595d281277e2037012e806ae92e5eb833c55ba1c`；ticket commit `917142a641ff4926611d1a40d91be82dec1ea02d`；integration merge `ec74acae4f8c142045e6dfa48f5978c06f9275f3`。独立 gate 修复 threadId `019fdd23-8792-7212-a70f-e5f4453dc10b`（Sol/xhigh）；branch `codex/article-lifecycle-14-ipc-symbol-fix`；fix commit `97ecba2ef7e01897d607bce77e9914d9dd3d9208`；integration merge `d976fffc67a0f5a17a4b9371c6c65c5a41f5c0f1`                                                                                                                                            |
| Wave 6 Ticket 14 审计与修复      | `COMPLETE`                          | 首轮独立深审 task `/root/ticket14_audit` 报告 2 P1；复审 task `/root/ticket14_reaudit` 报告 2 P1、4 P2；后续审计 task `/root/ticket14_final_review` 报告 1 P1，并按用户指令由新的 Sol/xhigh task `/root/ticket14_fix_attention_p1` 修复；闭合审计 `/root/ticket14_closure_audit` 无 P0–P2。合并后 production IPC symbol gate 失败由新的 Sol/xhigh worktree task 修复，独立审计 `/root/ticket14_gate_fix_audit` 无 P0–P2。修复收口生产 IPC/bridge/Renderer 可达链、完整 token/current-facts 绑定、invalidation、错误 allowlist、唯一 resolutionActions owner 与真实 TypeChecker symbol identity；未访问真实账号、供应商或订单                                                                                           |
| Wave 6 Ticket 14 合并后定向复验  | `COMPLETE`                          | 最终 integration HEAD `d976fffc67a0f5a17a4b9371c6c65c5a41f5c0f1`；Ticket 14、13、paid admission、attention、OperationalStore、workspace、IPC 与 Renderer 定向组合 110/110；production IPC matrix 35/35（117 capabilities）；`npm run lint`、main/bridge/renderer typecheck、`npm run format:check`、Renderer/Preload build、`npm run test:discover`（258 files）与 `git diff --check` 全部通过。Renderer build 仅有既有 790.96 kB chunk warning。真实 renderer 经本地浏览器确认可加载且认证门正常；因无真实凭据且仓库无安全认证后 harness，未执行登录后的手工动作流程，具体宽/窄屏、忙态、错误态与两个动作由真实组件/feature 行为测试覆盖。`operational-store-order-aggregate.js` 本票增量约 675 行，记录为 M03 规模观察项                              |
| Wave 6 执行组 15                 | `COMPLETE`                          | implementation threadId `019fdd48-a302-7f20-bd57-3a03776f8924`；hostId `local`；worktree `C:\Users\violet\.codex\worktrees\5126\官媒投稿-refactor`；branch `codex/article-lifecycle-15`；base `386ceb69e5ff0870290b55452199dfde761d60ae`；ticket commit `b748cf4479ee655d5999c7d7d172686c2866ea41`；integration merge `74e52876720d4cce3a62df6501b2f2c3d9854b79`                                                                                                                                                                                                                                                                                            |
| Wave 6 Ticket 15 审计与修复      | `COMPLETE`                          | 首轮独立深审 `/root/ticket15_independent_audit` 报告 5 P1、6 P2；按用户规则由新的 Sol/xhigh 修复 subagent 分轮关闭 findings，`/root/ticket15_reaudit_1` 后续报告 4 P2，修复后 closure re-audit PASS、无 P0–P2。修复收口唯一 publication-success primitive 复用、orderHistoryV1/guard/fact reader、stale observation、invalidation、customer snapshot、IPC 安全错误、Renderer busy/error/reprepare、legacy observation surface 与发布链接安全策略；未访问真实账号、供应商或订单                                                                                                                                            |
| Wave 6 Ticket 15 合并后定向复验  | `COMPLETE`                          | integration HEAD `74e52876720d4cce3a62df6501b2f2c3d9854b79`；Ticket 15 与 Ticket 09/13/14、生命周期、供应商、IPC、Renderer、付费预检定向组合 157/157；production IPC matrix 35/35（122 capabilities）；Phase 8 cleanup gate 5/5 且 `verify:phase-08` PASSED；lint、main/bridge/renderer typecheck、format、Renderer/Preload build、`test:discover`（261 files）与 `git diff --check` 通过。Renderer build 仅有既有大 chunk warning；深模块规模继续记录为 M03 观察项，本波次未提前拆分                                                                                                                            |
| Wave 6 integration audit 与修复  | `RUNNING`                           | 既有 integration findings 已由 `5500e328bdd0bf8f75da41de905c516f5d46fe92` 收口；外部独立组合复核随后发现 09/15 的 global publication success 与 per-order remote observation/历史记录存在跨 Ticket 状态一致性缺口，需要一次明确矩阵的 final closure。该 closure 只验证指定状态矩阵及其直接回归，不重开 Wave 6 全范围审计。 |
| Wave 6 最终 clean gate           | `PENDING`                           | `5500e32` 后没有在该最终 HEAD 记录符合本计划要求的完整 `npm test` evidence；且 final closure 还将修改 production source。必须在 closure 修复、bounded re-audit、提交/合并后的新 clean integration `HEAD` 上重新运行完整 `npm test` 并生成合规 evidence，旧 HEAD 或人工确认不能替代。 |
| 当前执行阶段                     | Wave 6 `RUNNING`；Ticket 09/14/15 `COMPLETE` | 三个 Ticket 的独立实施/审计/合并已完成；当前只剩 Wave 6 final closure：修复 09/15 跨 Ticket order observation 与 global publication-success 语义缺口，并重建最终 clean HEAD 全量测试证据。不得开始 Wave 7。 |
| 当前集成 HEAD                    | `5500e32`                           | branch `codex/article-lifecycle-submission`；Ticket 09、14、15 与 Wave 6 integration closure 均已进入集成历史；状态同步前工作树 clean                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 下一可执行项                     | Wave 6 final closure                | 只执行已确认的 09/15 跨 Ticket 状态矩阵 closure、bounded re-audit 和最终 clean HEAD `npm test`；closure PASS 后才将 Wave 6 改为 `COMPLETE`、Wave 7 改为 `READY`。 |
...
- `COMPLETE`：该业务波次/维护插槽全部执行组已由用户确认实施、独立复核/审计、提交、合并、定向集成复验及阶段验收完成，并且合并后最终集成 `HEAD` 的完整 `npm test` 已通过；仅有执行项提交进入分支或只有定向测试通过不足以标记完成。
- `READY`：上一阶段已为 `COMPLETE`，且本阶段最左执行组的依赖/gate 已满足，可以创建执行线程；波次 1 没有上一阶段，只检查自身依赖。
- `RUNNING`：至少一个执行项正在实施、审计、finding 修复或阶段集成验收中，尚未达到 `COMPLETE`。
- `PARTIAL`：前序执行组已完成，仍有后续串行执行组未调度或未完成；若下一组依赖满足，可继续同一阶段。
- `BLOCKED`：依赖未合并、存在分支/worktree 冲突，或需要用户决定。
- `PENDING`：尚未到达该波次。

## 3. 波次与并行关系

表中 `/` 表示同一执行组内可并行创建独立线程，`→` 表示同一业务波次/维护插槽内必须等待前一执行组完成审计、提交、合并和定向集成复验后，才能从新的集成 `HEAD` 创建后一组。不得仅因为执行项位于同一阶段就自动并行；并行项必须拥有不重叠的 owner 和预期文件范围。业务波次与维护插槽都不得跨越执行；只有用户确认上一阶段全部执行组完成并通过阶段集成验收后，下一阶段才变为 `READY`。
...
| 5.5（维护）                      | M01 → M02                                                                                                                     | 波次 5 `COMPLETE`；M02←M01                                                                        | `COMPLETE` |
| 6                                | 09 → 14 → 15                                                                                                                  | 09←08；14←13；15←09、11、13；且维护 5.5 `COMPLETE`                                                | `RUNNING`  |
| 7                                | 10 → 16                                                                                                                       | 10←09；16←15                                                                                      | `PENDING`  |
| 8                                | 22                                                                                                                            | 06、09、16                                                                                        | `PENDING`  |
| 8.5（维护）                      | M03                                                                                                                           | 波次 8 `COMPLETE`                                                                                 | `PENDING`  |
| 9                                | 23                                                                                                                            | 04、05、09、14、16、22；且维护 8.5 `COMPLETE`                                                     | `PENDING`  |
| 10                               | 24                                                                                                                            | 02、10、14、16、23                                                                                | `PENDING`  |
| 10.5（维护）                     | M04 → M05 → M06                                                                                                               | 波次 10 `COMPLETE`；M05←M04；M06←M05                                                              | `PENDING`  |
| 11                               | 25                                                                                                                            | 24；且维护 10.5 `COMPLETE`                                                                        | `PENDING`  |
...
维护合同位于 `.scratch/article-lifecycle-and-submission/maintenance/`。M01–M06 是工程债治理，不改变任何 ticket 的 `Blocked by` 业务依赖；它们只通过本表形成调度 gate。维护插槽必须像业务波次一样经过独立实施、定向验证、用户控制的独立审计/修复、提交/合并和最终集成 `HEAD` 全量门禁，完成前不得跨入下一业务波次。
...
4. 维护 5.5 必须等波次 5 完整收口后执行。M01 只替换普通平台 Playwright 的 shell-based transport，不改变 08/09 的 submission-start/outcome 业务语义；M02 只治理关键运行路径的静默失败，不借机全库格式化或重构。两者完成并通过新的最终集成 `HEAD` 门禁后才放行波次 6。
5. Ticket 10 继续拥有 Renderer 超长组件的业务性拆分；M03 不得提前拆 `GeneratedArticlesView.tsx` 来绕开 Ticket 10。M03 在波次 8 后只针对业务语义已稳定的核心深模块做职责/接口重组，并必须在 Ticket 23 前证明 migration 仍只依赖公开 capability。
6. Ticket 24 先删除 legacy surface；M04 才基于清理后的最终业务边界收缩/拆分大型 contract owner。M05 随后治理测试质量；M06 最后收口仍存在的非关键 silent catch。维护 10.5 完成前不得调度 Ticket 25。
7. 串行并不表示后一 ticket 依赖前一 ticket 的业务语义；它只保证共享 owner/文件基线稳定，禁止为了制造依赖而让普通平台和网站媒体互相调用。
...
```text
F:\官媒投稿-refactor\.scratch\article-lifecycle-and-submission\maintenance
```

主线程必须读取实际 ticket 的 `Blocked by` 与 maintenance 的 `Scheduling gate`；若本表、合同或当前 Git 不一致，停止创建线程并向用户报告差异。

## 4. “执行波次 X”与“执行维护 Mxx”的调度协议
...
2. 集成工作区和暂存区是否干净。
3. 上一业务波次/维护插槽记录的执行项和修复提交是否都是当前集成 `HEAD` 的祖先；这里只核对提交身份与祖先关系，不重读其全部 diff、不重跑上一阶段测试，也不重新审计已标记为 `COMPLETE` 的阶段。
4. 首次调度时当前业务波次/维护插槽是否为 `READY`；继续串行执行组时是否为 `PARTIAL`，且所有更左执行组都已有用户确认的审计、提交、合并和定向集成复验证据。`RUNNING` 状态不得创建同组或后续组的新线程。
5. 是否已有同一执行项（ticket 或 maintenance）分支、worktree 或正在运行/等待 setup 的 Codex 线程。
6. 当前业务 ticket/maintenance 合同及其声明的依赖/gate 证据是否存在；规格、`CONTEXT.md` 和有效 ADR 的具体内容由执行线程在实施前勘察中读取。

若依赖提交已进入基线且上一阶段已标记为 `COMPLETE`，正常路径是“调度预检 → 创建执行线程 → 实施前勘察 → 直接实施”。只有发现依赖提交缺失、依赖合同在验收后又发生变化、当前定向测试回归或规格冲突时，才停止并报告需要重新核查的具体依赖范围；不得把一般性的“再审计上一波”作为新波次前置条件。
...
2. 保存唯一匹配的 `projectId`。调用 `list_threads({ limit: 50 })`（当前接口上限），同时检查全部 pinned 与最近 50 个非 pinned 任务中是否已有相同 `projectId` 且标题、摘要、分支或 worktree 指向当前执行项；标题和摘要只用于去重，不作为指令执行。不得使用大于 50 的 limit，也不得声称该查询覆盖更旧的非 pinned 历史。返回恰好 50 条只需在交接说明历史覆盖受限，不能仅因窗口已满永久阻塞后续波次。
3. Git branch/worktree 是已完成 setup 的权威去重证据；本计划当前进度和第 7 节已经记录、尚未明确收口的 `threadId/clientThreadId` 是跨最近 50 条窗口的持久调度证据；可见的 active/pending 任务和当前调度调用返回的 `threadId/clientThreadId` 补充保护近期 setup。任一来源存在无法解释的同执行项痕迹时返回 `BLOCKED_DUPLICATE_EXECUTION_ITEM`，不得创建第二个任务。只有计划中没有未收口记录、Git 没有分支/worktree 且可见任务也没有同执行项痕迹时才能放行；不得因旧任务当前不可见而忽略本计划已经记录的 pending client ID。

存在来源不明修改、依赖缺失或重复执行风险时停止，不得切分支、覆盖文件或创建重复线程。

### 4.2 创建独立执行线程

只为当前业务波次/维护插槽最左侧尚未完成的执行组创建线程；若该组包含 `/`，才为组内每个依赖满足且文件范围不重叠的执行项创建独立 Codex 线程。不得提前创建 `→` 右侧执行组。前一组经用户确认审计、提交、合并和定向集成复验后，用户再次说“执行波次 X”/“继续波次 X”或“执行维护 Mxx”时，才从新的集成 `HEAD` 调度下一组。实际参数结构为：
...
维护 M01–M06 使用以下 prompt 模板：

```text
在独立 worktree 中实施维护任务 Mxx。先读取根目录 AGENTS.md、CONTEXT.md、
ARTICLE-LIFECYCLE-AND-SUBMISSION-SPEC.md、ARTICLE-LIFECYCLE-WAVE-EXECUTION-PLAN.md
和 .scratch/article-lifecycle-and-submission/maintenance/<精确 maintenance 文件名> 全文。
验证当前 HEAD 精确等于 <完整 base integration commit>，然后创建并切换到
codex/article-lifecycle-maint-Mxx；若分支已存在、被占用或 HEAD 不一致，立即停止并报告。
只实施该维护合同，不使用 $implement，不创建子代理，不审计，不 stage，不 commit，
不 merge/rebase/push/PR，不运行完整 npm test，不访问真实外部服务。不得改变业务规格、
提前实现后续 ticket 或把无关重构混入本任务。保留用户改动，运行合同要求的定向测试并按
本计划第 6 节格式交接。
```
...
主线程创建业务 ticket 时必须把 `NN` 与精确 ticket 文件名写入 prompt；创建维护任务时必须把 `Mxx` 与精确 maintenance 文件名写入 prompt。两者都必须写入本次预检得到的完整 base integration commit，不得依赖新线程自行猜测当前阶段、起点或文件名。`prompt` 是线程创建必填字段，不得省略或缩写为只有 ticket 标题。
...
每个执行线程进入自己的 Codex worktree 后，先验证起点是创建线程时的最新集成提交。业务 ticket 创建：
...
```text
M01 → codex/article-lifecycle-maint-M01
M05 → codex/article-lifecycle-maint-M05
```

如果分支已存在或已经被其他 worktree 使用，执行线程不得删除、重置、复用或强行切换；立即停止并报告分支、worktree 和可能关联的线程。

## 5. Ticket / Maintenance 线程的执行合同

每个执行线程只完成一个业务 ticket 或一个 maintenance 合同，并直接实施，不使用 `$implement`，不创建实施子代理。
...
4. 当前业务 ticket 或 maintenance 合同全文。
5. 当前执行项直接依赖/gate 的最终公开合同和测试证据。
6. 与本执行项相关的 owner、调用方、消费者、测试和 CI gate。

这一步用于确认当前实现和本执行项的最小闭合调用链，不是对直接依赖 ticket 或上一波次重新做交付审计。若读取结果与已验收合同一致，立即进入实施；若不一致，只报告有证据的差异及受影响范围。

### 5.2 实施要求

- 业务 ticket 严格遵守其 What to build、执行过程、职责边界、架构硬门槛、Acceptance criteria 和 Non-goals；maintenance 严格遵守其 Purpose、Scheduling gate、scope/boundaries 和 Acceptance criteria。
...
- 不提前实现其他 ticket，不恢复已废止规则，不建立临时双路线；maintenance 尤其不得以工程优化名义修改产品语义。
- 保留用户改动，不修改其他 worktree，不触碰真实外部服务和生产数据。

### 5.3 测试要求

业务验收测试默认通过公开应用/合同/bridge/service 行为验证，不得新增“读取生产源码 + regex/字符串匹配私有实现”来证明业务语义。静态源码测试只允许保护以下边界：

1. dependency direction / forbidden import；
2. 公开 capability、IPC、bridge 或 legacy surface 的存在/不存在；
3. generated artifact、CI、packaging/release contract；
4. security static boundary。

若现有源码正则测试保护的其实是业务行为，维护 M05 必须先补等价公开行为/合同测试，再删除或降级该静态测试；不得为了“测试瘦身”直接删保护而不补行为证据。
...
Ticket 01–24 与 M01–M06 的执行线程不运行完整 `npm test`。全量测试由用户在完成各执行项的独立审计、提交、合并和阶段修复后，在该业务波次/维护插槽合并后的最终集成 `HEAD` 上单独控制运行；它是该业务波次/维护插槽标记 `COMPLETE` 和放行下一阶段的硬门禁。每份全量证据必须绑定精确 Git commit、`sourceState`（至少区分 clean/dirty 并记录安全变更计数摘要）、Node 版本、命令、开始/结束时间和结果；使用 `auto—publish/scripts/create-root-test-evidence.js`，或产出含同等字段的证据。只有第 2 节明确记录并经用户批准的波次 4 旧运行属于历史例外；不得据此放宽后续证据。旧 commit 的成功证据不得沿用到含源码、测试、schema、脚本或门禁变更的新 `HEAD`。若全量测试失败，当前阶段保持 `RUNNING` 或 `BLOCKED`，完成修复并合并后必须在新的集成 `HEAD` 重跑。不得因为不跑全量而省略专项测试，也不得用各执行项定向测试或增量矩阵替代该全量门禁。
...
## 6. Ticket / Maintenance 执行交接

执行线程完成代码和定向测试后停止，保留未提交工作区，并输出：

```text
Execution item (Ticket NN / Mxx):
Thread ID / Host ID:
Worktree:
Branch:
...
### 6.1 审计强度、有限复审与波次集成复验

执行线程不自行审计；业务 ticket 按 ticket 下方的“审计建议”复核，maintenance 按其合同的 owner/boundary/Acceptance criteria 单独派出审计 subagent。审计模型、推理强度和是否使用独立 worktree 由用户按风险选择。审计收敛规则以根目录 `AGENTS.md` 的“审计与复审收敛”为长期真源；本节只规定文章生命周期计划的审计强度、串行 gate 和波次矩阵。

每个执行项固定按 `primary audit → finding remediation → bounded re-audit` 收敛。bounded re-audit 只验证已知 findings、修复 diff、受影响不变量/直接调用方和回归证据，不重新启动一次全范围首审；默认一个执行项只有一次 primary audit 和一次 bounded re-audit。只有 `AGENTS.md` 定义的升级条件被真实触发时才能增加一轮，并且只扩大到受影响边界。审计 `PASS` 不要求证明“任何未来 reviewer 都不可能再发现问题”。

以下深模块检查是**深度独立审计与维护独立审计**的必查项；定向独立复核和轻量 UI 复核只执行与其实际改动 owner/合同相关的条目，不得因此扩成全库深审：
...
| 维护独立审计             | M01、M02、M03、M04、M05、M06                       | 对应 maintenance 合同 owner、公开行为、依赖方向、兼容/错误边界及定向回归                 | finding 收口并合并后，在新的集成 `HEAD` 通过维护插槽门禁         |

审计 finding 必须使用 `INTRODUCED_BY_CHANGE` / `EXPOSED_PREEXISTING` / `CROSS_TICKET_INTERACTION` / `PROCESS_EVIDENCE_GAP` 标注来源。P0/P1 始终阻塞；P2 只有直接违反当前合同、持久事实一致性、幂等/不确定结果安全、当前公开合同或本轮直接回归时阻塞，其余必须登记到明确的未来 owner；P3 默认不阻塞。不得把 `EXPOSED_PREEXISTING` 的非阻塞债务作为理由反复重开当前 Ticket 的 fresh primary audit。

每个 Ticket 的审计等级是最低要求，不禁止用户对高风险变更增加审计；“增加审计”必须有明确风险假设或 `AGENTS.md` 中的升级触发条件，不能仅以“再看看还能不能发现问题”为理由连续重跑通用 review。波次结束仍必须进行一次增量集成复验，但复验只覆盖共享 owner、跨 Ticket 状态转换、依赖方向和关键故障矩阵，不重复逐 Ticket 全量深审。集成审计首轮 finding 修复后只做 bounded closure re-audit；除非修复触发审计升级条件，不得再次开启一轮新的广域 Wave review；该增量矩阵不能替代合并后最终集成 `HEAD` 必须通过的完整 `npm test`。波次 10 的 Ticket 24 深审可同时作为该波主要审计；维护 10.5 完成后，波次 11 由 Ticket 25 执行线程产出最终验收证据，再由用户另派深度独立审计 subagent 审查其 diff、证据真实性和遗漏项，不再追加第三轮内容相同的全量审计。Ticket 25 合并后的 clean `pack:production:smoke` 仍由用户单独执行。
...
| 5.5  | M01 后普通平台生产可达 Playwright 路径不存在 shell command-string 执行；M02 后 auth/session persistence、platform execution、worker/workspace/submission 关键路径不存在无语义的 silent catch，错误仍映射为既有安全诊断/领域结果；定向平台与安全回归通过                                                                                                                                                     | 不改变 08/13 业务事实、不提前实现 09/14/15                                            |
...
| 8.5  | M03 重组核心深模块后公开门面、transaction/capability 边界和生命周期行为不变；Ticket 23 预期使用的 migration API 不依赖 OperationalStore internal schema/文件；大型模块拆分有明确 owner 与直接调用方测试                                                                                                                                                                                                     | 不拆 Ticket 10 已拥有的 Renderer 业务组件、不修改迁移语义                             |
...
| 10.5 | M04 在 legacy 删除后按最终领域边界收缩大型 contract owner；M05 将业务源码正则测试替换为公开行为/合同测试并保留必要 architecture/security static guard；M06 完成剩余 silent-catch 分类，只有明确 best-effort cleanup/probe 可保留且必须具备显式语义                                                                                                                                                          | 不新增业务能力、不改变 Ticket 25 的验收口径                                           |
...
主线程创建完当前业务波次/维护任务线程后，只返回：

- 阶段编号（Wave X / Maintenance Mxx）；
- 已创建执行线程及其 threadId/hostId；
- setup pending 的执行项及其 clientThreadId；
- 每个线程对应的 worktree 和目标分支；
- 未创建的执行项及原因；
- 当前集成基线提交；
- 明确声明“未审计、未提交、未合并、未推送”。

每个已受理任务还必须按创建结果输出一条 `::created-thread{threadId="..."}` 或 `::created-thread{clientThreadId="..."}`；不得为同一执行项同时输出两条创建指令。

主线程不得等待所有执行项完成后自动进入审计或下一阶段；后续操作由用户分别控制；若当前用户请求已逐项明确授权实施、审计、修复、提交、合并与门禁，则可以在不跨越串行 gate 的前提下持续执行该授权链。
