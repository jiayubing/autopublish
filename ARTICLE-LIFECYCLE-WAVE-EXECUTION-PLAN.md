...
| 当前执行波次                    | `6` / `READY`                                                         | 波次 5 的 Ticket 08、Ticket 13、独立审计与修复、增量集成复验、收敛提交、clean production smoke、完整测试失败修复及最终 clean provenance 全量验收均已完成；波次 5 已关闭，波次 6 可以按 09 → 14 → 15 的顺序开始                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 当前集成 HEAD                   | `b3d89a6910b50f34ce6b2f16c36dc3c2338197f0`                            | branch `codex/article-lifecycle-submission`；commit `test: align wave 5 regression contracts`。最终全量验收运行前后 sourceState 均为 `CLEAN`；更新本计划后仅本计划文档为 tracked 修改，暂存区与 untracked 为空                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 下一可执行波次                  | 波次 6 / `READY`                                                      | 首个执行组为 Ticket 09；Ticket 09 完成用户确认的审计、修复、提交、合并和定向集成复验后，才可从新的集成 `HEAD` 调度 Ticket 14，随后按同一规则推进 Ticket 15                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 6                                | 09 → 14 → 15                                                                                                                  | 09←08；14←13；15←09、11、13                                                                       | `READY`    |
